...
class KlnResepModel extends CI_Model
{
	...
	{
		$id = (int)$id;
		$this->db->where('kl_resep_id', $id);
		$this->db->delete('kln_resep_dt');
		$this->db->where('id', $id);
		$this->db->delete('kln_resep_ht');
		return true;
	}
	...
	public function update($id, $input)
	{
		...
		$this->db->where('id', $id);
		$this->db->update('kln_resep_ht', $ht);
		...
		foreach ($details as $key => $value) {
			$this->db->insert("kln_resep_dt", array(
				'resep_id' => $id,
	...
	public function retrieve($id)
	{
		...
		$this->db->where('id', $id);
		$result = $this->db->get('kln_resep_ht', 1);
	...
	public function retrieve_detail($hdid)
	{
		...
		$this->db->from('kln_resep_dt a');
		$this->db->join('inv_item b', 'a.item_id = b.id', 'Left');
		$this->db->where('a.resep_id', $hdid);
		$res = $this->db->get();
	...
	public function create($input)
	{
		...
		$this->db->insert('kln_resep_ht', $ht);
