...
class InvPindahGudangModel extends CI_Model
{
	...
	public function create(
		...
		$data = array(
			...
			'inv_permintaan_pindah_gudang_id' => $input['inv_permintaan_pindah_gudang_id']['id'],
			'tipe' => $input['tipe']['id'],
		...
		$this->db->insert('inv_pindah_gudang_ht', $data);
		$id = $this->db->insert_id();

		$details = $input['details'];
		foreach ($details as $key => $value) {
			$this->db->insert("inv_pindah_gudang_dt", array(
				'inv_pindah_gudang_id' => $id,
	...
	public function update($id,	$input)
	{
		...
		$data['inv_permintaan_pindah_gudang_id'] = $input['inv_permintaan_pindah_gudang_id']['id'];
		...
		// hapus  detail
		$this->db->where('inv_pindah_gudang_id', $id);
		$this->db->delete('inv_pindah_gudang_dt');
	...
	public function posting($id,	$input)
	{
		...
		$this->db->where('id', $id);
		$this->db->update('inv_pindah_gudang_ht', $data);
		$data_transaksi = $this->db->query("select * from inv_pindah_gudang_ht a
		inner join inv_pindah_gudang_dt b on a.id=b.inv_pindah_gudang_id 
		...
		foreach ($data_transaksi as $key => $value) {
			...
			$stok_akhir = $this->db->query("SELECT  SUM(qty_masuk-qty_keluar)AS qty,SUM(nilai_masuk-nilai_keluar)AS nilai from inv_transaksi_harian
					where item_id=" . $value['item_id'] . " and gudang_id=" . $value['dari_gudang_id'] . " ")->row_array();
			...
			$item_dt = $this->db->query("select * from inv_item_dt  where gudang_id=" . $value['dari_gudang_id'] . "
			  and item_id=" . $value['item_id'] . "")->row_array();
			$avg_price = 0;
			if ($item_dt) {
				$avg_price = $item_dt['nilai'] / $item_dt['qty'];
				$this->db->where('item_id', $value['item_id']);
				$this->db->where('gudang_id', $value['dari_gudang_id']);
				$this->db->update("inv_item_dt", array(
					'item_id' => $value['item_id'],
					'gudang_id' => $value['dari_gudang_id'],
	...
	public function delete($id)
	{
		...
		$this->db->delete('inv_pindah_gudang_dt');
		$this->db->where('id', $id);
		$this->db->delete('inv_pindah_gudang_ht');
		return true;
	}
	...
	public function retrieve_all()
	{
		...
		$result = $this->db->get('inv_pindah_gudang_ht');
	...
	{
		$this->db->from('inv_pindah_gudang_dt');
	...
	{
		$this->db->select('*');
		$this->db->from('inv_pindah_gudang_ht');
		$this->db->where('id', $id);
		$res = $this->db->get();
	...
	public function retrieve_detail($hdid)
	{
		// $this->db->select('*');
		// $this->db->from('inv_pindah_gudang_dt');
		// $this->db->where('inv_pindah_gudang_id', $hdid);
		// $res = $this->db->get();
		// return $res->result_array();
		$query = "select a.*,e.acc_akun_id, b.kode as kode_barang,b.nama as nama_barang,d.kode as uom from inv_pindah_gudang_dt a
		...
		where a.inv_pindah_gudang_id=" . $hdid . "";
