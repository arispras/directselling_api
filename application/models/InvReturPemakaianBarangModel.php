...
class InvReturPemakaianBarangModel extends CI_Model
{
	...
	public function create(
		...
		$data = array(
			'inv_pemakaian_id' => $input['inv_pemakaian_id']['id'],
		...
		$this->db->insert('inv_retur_pemakaian_ht', $data);
		$id = $this->db->insert_id();

		$details = $input['details'];
		foreach ($details as $key => $value) {
			$this->db->insert("inv_retur_pemakaian_dt", array(
				'inv_retur_pemakaian_id' => $id,
	...
	{
		$id = (int)$id;
		$data['inv_pemakaian_id'] = $input['inv_pemakaian_id']['id'];
		...
		// hapus  detail
		$this->db->where('inv_retur_pemakaian_id', $id);
		$this->db->delete('inv_retur_pemakaian_dt');
	...
	public function posting($id,	$input) 
	{
		...
		$this->db->where('id', $id);
		$this->db->update('inv_retur_pemakaian_ht', $data);
		$data_transaksi=$this->db->query("select * from inv_retur_pemakaian_ht a
		inner join inv_retur_pemakaian_dt b on a.id=b.inv_retur_pemakaian_id  
		...
		foreach ($data_transaksi as $key => $value) {
			$inv_tr = $this->db->query("select * from inv_transaksi_harian  where tipe='PEMAKAIAN' and ref_id=" . $value['inv_pemakaian_id'] . "
			...
			// 			'nilai' =>  $item_hpp['nilai'] + ($value['qty'] * $avg_price),
			// 			'qty_masuk' => $value['qty'],
			// 			'nilai_masuk' => ( $value['qty'] *$avg_price),
	...
	public function delete($id)
	{
		...
		$this->db->delete('inv_retur_pemakaian_dt');
		$this->db->where('id', $id);
		$this->db->delete('inv_retur_pemakaian_ht');
		return true;
	}
	...
	public function retrieve_all()
    {
        ...
        $result = $this->db->get('inv_retur_pemakaian_ht');
	...
	{
		$this->db->from('inv_retur_pemakaian_dt');
		$this->db->where('inv_retur_pemakaian_id', $id);
	...
	{
		$this->db->select('*');
		$this->db->from('inv_retur_pemakaian_ht');
		$this->db->where('id', $id);
		$res = $this->db->get();
		return $res->row_array();
	}

	public function retrieve_by_pemakaian_posting($id)
	{
		$this->db->select('*');
		$this->db->from('inv_retur_pemakaian_ht');
		...
		$this->db->select('*');
		$this->db->from('inv_retur_pemakaian_dt');
		$this->db->where('inv_retur_pemakaian_id', $res['id']);
		$res['detail'] = $this->db->get()->row_array();
		return $res;
	}
	...
	public function retrieve_detail($hdid)
	{
		...
		FROM inv_retur_pemakaian_ht a inner join inv_retur_pemakaian_dt b on a.id=b.inv_retur_pemakaian_id 
