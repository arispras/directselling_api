...
class WrkKegiatanModel extends CI_Model
{
	...
	public function create(
		...
		$data = array(
			...
			'alasan' => $input['alasan'],
			'kendaraan_mesin_id' =>  $input['kendaraan_mesin_id']['id'],
		...
		$this->db->insert('wrk_kegiatan_ht', $data);
		$id = $this->db->insert_id();
		
		$details = $input['details'];
		foreach ($details as $key => $value) {
			$this->db->insert("wrk_kegiatan_dt", array(
				'wrk_kegiatan_id' => $id,
		...
		foreach ($details as $key => $value) {
			$this->db->insert("wrk_kegiatan_log", array(
				'wrk_kegiatan_id' => $id,
		...
		foreach ($details_item as $key => $value) {
			$this->db->insert("wrk_kegiatan_item", array(
				'wrk_kegiatan_id' => $id,
	...
	public function update(
		...
		// hapus  detail
		$this->db->where('wrk_kegiatan_id', $id);
		$this->db->delete('wrk_kegiatan_dt');

		// hapus  detail item
		$this->db->where('wrk_kegiatan_id', $id);
		$this->db->delete('wrk_kegiatan_item');

		$this->db->where('wrk_kegiatan_id', $id);
		$this->db->delete('wrk_kegiatan_log');
		...
		foreach ($details as $key => $value) {
			$this->db->insert("wrk_kegiatan_log", array(
				...
				'kendaraan_id' => $value['kendaraan_id']['id'],
				'blok_id' => $value['blok_id']['id'],
				'acc_kegiatan_id' => $value['acc_kegiatan_id']['id'],
				'jumlah_jam' => $value['jumlah_jam'],
				'volume' => $value['volume'],
				'ket' => $value['ket'],
			));
	...
	public function delete($id)
	{
		...
		$this->db->delete('wrk_kegiatan_log');
		$this->db->where('id', $id);
		$this->db->delete('wrk_kegiatan_ht');
		return true;
	}
	...
	{
		$this->db->select('*');
		$this->db->from('wrk_kegiatan_dt');
		$this->db->where('wrk_kegiatan_id', $hdid);
		$res = $this->db->get();
		return $res->result_array();
	}
	
	public function retrieve_log($hdid)
	{
		$this->db->select('*');
		$this->db->from('wrk_kegiatan_log');
		$this->db->where('wrk_kegiatan_id', $hdid);
		$res = $this->db->get();
		return $res->result_array();
	}
	...
	{
		$this->db->select('*');
		$this->db->from('wrk_kegiatan_item');
		$this->db->where('wrk_kegiatan_id', $hdid);
		$res = $this->db->get();
		return $res->result_array();
	}
	...
	public function print_slip(
		...
		FROM wrk_kegiatan_ht a inner join wrk_kegiatan_dt b
	   on a.id=b.wrk_kegiatan_id
