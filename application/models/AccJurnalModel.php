...
class AccJurnalModel extends CI_Model
{
	...
	public function create(
		...
		$this->db->insert('acc_jurnal_ht', $data);
		$id = $this->db->insert_id();

		$details = $input['details'];

		foreach ($details as $key => $value) {
			$this->db->insert("acc_jurnal_dt", array(
				'jurnal_id' => $id,
	...
	public function delete_by_ref_id_and_modul($ref_id, $modul)
	{
		...
		foreach ($jurnal as $key => $j) {
			...
			$this->db->delete('acc_jurnal_dt');

			$this->db->where('id', $j['id']);
			$this->db->delete('acc_jurnal_ht');
		}
	}
	public function update(
		...
		$this->db->where('id', $id);
		$this->db->update('acc_jurnal_ht', $data);
	...
	{
		$lastnumber = $this->db->query("select  max(no_jurnal)as last from acc_jurnal_ht ")->row_array();
