...
class KlnRawatJalanModel extends CI_Model
{
	...
	public function delete($id)
	{
		...
		$this->db->where('id', $id);
		$this->db->delete('kln_rawat_jalan');
		return true;
	}
	...
	{
		$id = (int)$id;
		$this->db->select('kln_rawat_jalan.*,kln_rawat_jalan.id as id,kln_pasien.nama as nama_pasien,
		kln_pasien.nip as id_pasien,karyawan.nama as nama_dokter,kln_pasien.jenis_kelamin,kln_pasien.tgl_lahir
		,kln_pasien.foto');
		$this->db->from('kln_rawat_jalan');
		$this->db->join('kln_pasien', 'kln_rawat_jalan.pasien_id=kln_pasien.id');
		$this->db->join('karyawan', 'kln_rawat_jalan.dokter_id=karyawan.id');
		$this->db->where('kln_rawat_jalan.id', $id);
		$result = $this->db->get();
	...
	{
		$query  = "select a.*,b.nama as nama_lokasi, e.nama as lokasi,c.nama as nama_asien  from kln_rawat_jalan a 
		left join gbm_organisasi b on a.lokasi_id=b.id 
		left join kln_pasien c on a.pasien_id=c.id 
	...
	public function posting($id, $input)
	{
		...
		$this->db->where('id', $id);
		$this->db->update('kln_rawat_jalan', $data);
		return true;
	}
	...
	public function create(
		...
		$this->db->insert('kln_rawat_jalan', $data);
