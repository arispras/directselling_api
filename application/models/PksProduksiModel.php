...
class PksProduksiModel extends CI_Model
{
    ...
    {
        $this->db->select("
            a.*,a.tanggal as tanggal_produksi,
            b.*,
            a.id AS id,
            b.nama AS lokasi,
			c.*,c.tanggal as tanggal_lab
        ");
        $this->db->from("pks_produksi_harian a");
        $this->db->join("gbm_organisasi b", "a.mill_id = b.id");
		$this->db->join("pks_lab_pengolahan c", "a.tanggal = c.tanggal","left");
