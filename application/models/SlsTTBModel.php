...
class SlsTTBModel extends CI_Model
{
	...
	public function retrieve_all(
		...
		$data = $this->pager->set('sls_ttb_ht', $no_of_records, $page_no, $where);
	...
	public function delete($id)
	{
		...
		$this->db->delete('sls_ttb_dt');
		$this->db->where('id', $id);
		$this->db->delete('sls_ttb_ht');
		return true;
	}
	...
	public function create($input)
	{
		...
		foreach ($details as $key => $value) {
			$this->db->insert("sls_ttb_dt", array(
				'ttb_hd_id' => $id,
	...
	public function retrieve($id)
	{
		...
		$this->db->select('a.*, b.nama_customer,c.no_so,c.tanggal as tanggal_so');
		$this->db->from('sls_ttb_ht a');
		$this->db->join('gbm_customer b', 'a.customer_id = b.id', "left");
		$this->db->join('sls_so_ht c', 'a.sls_so_id = c.id', "left");
	...
	public function retrieve_so_dtl_blm_terima2($so_id)
	{
		...
		b.qty-IFNULL(e.qty_terkirim, 0)as qty_belum_terima ,  b.harga,b.diskon 
			from sls_ttb_ht a INNER JOIN sls_ttb_dt b ON a.id=b.ttb_hd_id
