...
class M_DatatablesModel extends CI_Model
    {
        ...
        function get_tables_query($query,$cari,$where,$iswhere,$post)
        {
			$_POST=$post;
			// Ambil data yang di ketik user pada textbox pencarian
            $search = htmlspecialchars($_POST['search']['value']);
		...
		function get_tables_query_karyawan($query,$cari,$where,$iswhere,$post)
        {
			$_POST=$post;
			// Ambil data yang di ketik user pada textbox pencarian
            $search = htmlspecialchars($_POST['search']['value']);
		...
		foreach ($data as $karyawan) {
			$result = [];
			$sql_role = $this->db->query("SELECT * FROM karyawan_role  where karyawan_id= " . $karyawan['id'] . "");
			$res_role = $sql_role->result_array();
			$result=$karyawan;
			$result['role'] = $res_role;
