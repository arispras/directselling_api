...
class AccApInvoiceModel extends CI_Model
{

	public function create(
		$input = null
	) {
		$supp=$this->db->query("select * from gbm_supplier where id=".$input['supplier_id']['id']."")->row_array();
		...
		$data = array(
			...
			'no_invoice_supplier' => $input['no_invoice_supplier'],
		...
		$this->db->insert('acc_ap_invoice_ht', $data);
		$id = $this->db->insert_id();

		$details = $input['details'];

		foreach ($details as $key => $value) {
			$this->db->insert("acc_ap_invoice_dt", array(
				'invoice_id' => $id,
				'lokasi_id' =>$input['lokasi_id']['id'],
	...
	public function delete_by_ref_id_and_modul($ref_id ,$modul)
	{
		...
		$this->db->select('*');
		$this->db->from('acc_ap_invoice_ht');
	...
	{
		$this->db->select('*');
		$this->db->from('acc_ap_invoice_dt');
	...
	{
		$lastnumber = $this->db->query("select  max(no_transaksi)as last from acc_ap_invoice_ht ")->row_array();
	...
	public function print_slip(
		...
		FROM acc_ap_invoice_ht a inner join acc_ap_invoice_dt b
