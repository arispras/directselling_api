...
class KaryawanModel extends CI_Model
{
	...
	public function retrieve_all_karyawan_dokter()
	{

		// $this->db->order_by('nama', 'ASC');
		$this->db->select('a.*, a.id AS id, b.nama AS lokasi');
		$this->db->from('karyawan a');
		$this->db->join('gbm_organisasi b', 'a.lokasi_tugas_id = b.id', 'left');
		$this->db->where('a.is_dokter','1');
		$result = $this->db->get();
		return $result->result_array();
	}
	public function retrieve_all_karyawan_perawat()
	{
		...
		$this->db->from('karyawan a');
		$this->db->join('gbm_organisasi b', 'a.lokasi_tugas_id = b.id', 'left');
		$this->db->where('a.is_perawat',true);
		$result = $this->db->get();
		return $result->result_array();
	}
