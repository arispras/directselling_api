...
class KlnInvoiceRawatInapModel extends CI_Model
{
	...
	public function retrieve_all()
	{
		...
		$result = $this->db->get('kln_invoice_rawat_inap');
	...
	public function delete($id)
	{
		...
		$this->db->delete('kln_invoice_rawat_inap_dt');
		$this->db->where('id', $id);
		$this->db->delete('kln_invoice_rawat_inap_ht');
		return true;
	}
	...
	public function update($id, $input)
	{
		...
		$this->db->where('id', $id);
		$this->db->update('kln_invoice_rawat_inap_ht', $ht);
		...
		$this->db->where('invoice_id', $id);
		$this->db->delete('kln_invoice_rawat_inap_bayar');

		$details = $input['details'];
		foreach ($details as $key => $value) {
			$this->db->insert("kln_invoice_rawat_inap_dt", array(
				'invoice_id' => $id,
		...
		foreach ($detailsPembayaran as $key => $value) {
			$this->db->insert("kln_invoice_rawat_inap_bayar", array(
				'invoice_id' => $id,
	...
	public function posting($id,	$input)
	{
		...
		$this->db->where('id', $id);
		$this->db->update('kln_invoice_rawat_inap', $data);	
	...
	public function retrieve($id)
	{
		...
		$this->db->where('id', $id);
		$result = $this->db->get('kln_invoice_rawat_inap_ht', 1);
	...
	public function retrieve_detail($hdid)
	{
		...
		$this->db->from('kln_invoice_rawat_inap_dt a');
	...
	public function retrieve_detail_pembayaran($hdid)
	{
		...
		$this->db->from('kln_invoice_rawat_inap_bayar a');
	...
	public function create($input)
	{
		...
		$this->db->insert('kln_invoice_rawat_inap_ht', $ht);
