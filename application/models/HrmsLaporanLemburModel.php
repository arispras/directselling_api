...
class HrmsLaporanLemburModel extends CI_Model
{
    ...
    public function laporanLembur( $input='' )
    {
        $this->db->select("
            a.*,
            b.*,
        ...
        ");
        $this->db->from("payroll_lembur a");
        $this->db->join("karyawan b", "a.karyawan_id = b.id", 'left');
		// var_dump($input);exit();
        $this->db->where("a.karyawan_id", $input['karyawan_id']);
        // $this->db->where("a.lokasi_id", $input['lokasi_id']);

        $this->db->where("(a.tanggal) >= ", $input['tgl_mulai']);
        $this->db->where("(a.tanggal) <= ", $input['tgl_akhir']);
    ...
    public function laporanLemburBulanan( $input='' )
    {
        $this->db->select("
            a.*,
            b.*,
            a.id AS id,
            a.tanggal AS tanggal,
            c.nama AS sub_bagian
        ");
        $this->db->from("payroll_lembur a");
        $this->db->join("karyawan b", "a.karyawan_id = b.id", 'left');
        $this->db->join("gbm_organisasi c", "b.sub_bagian_id = c.id", 'left');
    ...
    public function laporanLemburJenis( $input='' )
    {
        $this->db->select("
            *
        ");
        $this->db->from("hrms_jenis_lembur");
