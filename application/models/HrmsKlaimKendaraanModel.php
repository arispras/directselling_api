...
class HrmsKlaimKendaraanModel extends CI_Model
{
	...
	public function create(
		...
		$this->db->insert('hrms_klaim_kendaraan_ht', $data);
		$id = $this->db->insert_id();

		$details = $input['details'];
		foreach ($details as $key => $value) {
			$this->db->insert("hrms_klaim_kendaraan_dt", array(
				'klaim_kendaraan_id' => $id,
				'nilai' => $value['nilai'],
				'ket' => $value['ket'],
			));
		}
	...
	public function update($id,	$input) 
	{
		...
		$this->db->where('id', $id);
		$this->db->update('hrms_klaim_kendaraan_ht', $data);

		// hapus  detail
		$this->db->where('klaim_kendaraan_id', $id);
		$this->db->delete('hrms_klaim_kendaraan_dt');
	...
	public function delete($id)
	{
		...
		$this->db->delete('hrms_klaim_kendaraan_dt');
		$this->db->where('id', $id);
		$this->db->delete('hrms_klaim_kendaraan_ht');
		return true;
	}
	...
	public function retrieve_all()
    {
        ...
        $result = $this->db->get('hrms_klaim_kendaraan_ht');
        return $result->result_array();
    }
	
	public function retrieve_all_detail($id)
	{
		$this->db->from('hrms_klaim_kendaraan_dt');
		$this->db->where('klaim_kendaraan_id', $id);
	...
	{
		$this->db->select('*');
		$this->db->from('hrms_klaim_kendaraan_ht');
		$this->db->where('id', $id);
		$res = $this->db->get();
	...
	public function retrieve_detail($hdid)
	{
		$this->db->select('*');
		$this->db->from('hrms_klaim_kendaraan_dt');
		$this->db->where('klaim_kendaraan_id', $hdid);
		$res = $this->db->get();
