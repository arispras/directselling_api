...
class HrmsPengajuanCutiModel extends CI_Model
{
	...
	public function retrieve_all(
		...
		$data = $this->pager->set('hrms_pengajuan_cuti', $no_of_records, $page_no, $where);
	...
	public function retrieve_all_kategori()
	{
		...
		$this->db->from('hrms_pengajuan_cuti a');
	...
	public function delete($id)
	{
		...
		$this->db->where('id', $id);
		$this->db->delete('hrms_pengajuan_cuti');
		return true;
	}
	...
	{
		$this->db->select('*');
		$this->db->from('hrms_pengajuan_cuti');
		$this->db->where('id', $id);
		$res = $this->db->get();
	...
	public function save_upload($inv, $file)
	{
		...
		$this->db->where('id', $id);
		$this->db->update('hrms_pengajuan_cuti', $data);
	...
	public function update($id, $input)
	{
		...
		$ht['dari_tanggal'] = $input['dari_tanggal'];
		$ht['sampai_tanggal'] = $input['sampai_tanggal'];
		$ht['cuti'] = $input['cuti'];
		$ht['lokasi_id'] = $input['lokasi_id']['id'];
		$ht['karyawan_id'] = $input['karyawan_id']['id'];
		$ht['jenis_absensi_id'] = $input['jenis_absensi_id']['id'];
		$ht['diubah_oleh'] = $input['diubah_oleh'];
		$ht['diubah_tanggal'] = date('Y-m-d H:i:s');
		$this->db->where('id', $id);
		$this->db->update('hrms_pengajuan_cuti', $ht);
	...
	{
		$id = (int)$id;
		$this->db->select('hrms_pengajuan_cuti.*,karyawan.sub_bagian_id');
		$this->db->from('hrms_pengajuan_cuti');
		$this->db->join('karyawan', 'hrms_pengajuan_cuti.karyawan_id = karyawan.id');
		$this->db->where('hrms_pengajuan_cuti.id', $id);
		$res = $this->db->get();
		return $res->row_array();
	...
	public function create($input)
	{
		...
		$this->db->insert('hrms_pengajuan_cuti', $ht);
	...
	public function retrieve_po_dtl_blm_terima($po_id)
	{
		...
		b.qty-IFNULL(e.qty_terima, 0)as qty_belum_terima ,  b.harga,b.diskon 
			from hrms_pengajuan_cuti a INNER JOIN prc_po_dt b ON a.id=b.po_hd_id
