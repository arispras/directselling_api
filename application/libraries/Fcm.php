...
class Fcm
{
	...
	/* Cara kirim notif versi Firebase messaging terbaru */
	public function kirimNotif($token,$title,$body,$data_send){
		$data=array();
		$data['to']=$token;
		$data['notification']['body']=$body;//"PP No xxx butuh approval";
		$data['notification']['title']=$title;//"Approval PP";
        $data['notification']['click_action']='FLUTTER_NOTIFICATION_CLICK';//"action ";
		$data['data']=$data_send;// harus array consth data['keterangan']="1234";
