...
class SlsSo extends BD_Controller
{
	...
	public function list_revisi_post()
	{
		...
		$query  = "SELECT a.*, b.nama AS lokasi,c.nama_customer as nama_customer,d.no_quotation,d.no_referensi from sls_so_ht a 
		left join gbm_organisasi b on a.lokasi_id=b.id
		left join gbm_customer c on a.customer_id=c.id
		-- LEFT JOIN inv_pengiriman_so_ht e ON a.id=e.so_id
		left join prc_quotation d on a.quotation_id=d.id";
		$search = array('no_so', 'a.tanggal', 'a.catatan', 'c.nama_customer', 'd.no_quotation', 'd.no_referensi');
		$where  = null;
	...
	public function listByUserApprove_post()
	{
		...
		from sls_so_ht a left join gbm_organisasi b on a.lokasi_id=b.id 
		...
		$search = array('no_so', 'a.tanggal', 'a.catatan', 'd.nama_customer', 'e.no_quotation', 'e.no_referensi');
	...
	public function listByUserApproveMobile_post()
	{
		...
		$query  = "SELECT a.*, b.nama AS lokasi,d.nama_customer as nama_customer,e.no_quotation,e.no_referensi from sls_so_ht a left join gbm_organisasi b on a.lokasi_id=b.id 
				inner join fwk_users c on a.last_approve_user=c.employee_id
				inner join gbm_customer d on a.customer_id=d.id
				left join prc_quotation e on a.quotation_id=e.id
	...
	public function countByUserApprove_post()
	{
		...
		if ($this->user_id) {
			$query  = "SELECT COUNT(*)as jumlah from sls_so_ht a left join gbm_organisasi b on a.lokasi_id=b.id 
				inner join fwk_users c on a.last_approve_user=c.employee_id
				where a.proses_approval=1 and a.status not in ('REJECTED','RELEASE') and c.id=" . $this->user_id;
	...
	{
		$id = $segment_3;
		$retrieve = $this->SlsSoModel->retrieve($id);
		$retrieve['detail'] = $this->SlsSoModel->retrieve_detail($id);
	...
	{
		$id = $segment_3;
		$retrieve = $this->SlsSoModel->retrieve_pembayaran_by_so($id);
	...
	{
		$id = $segment_3;
		$retrieve = $this->SlsSoModel->retrieve_invoice_by_so($id);
	...
	{
		$id = $segment_3;
		$retrieve = $this->SlsSoModel->retrieve_pembayaran_by_id($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->SlsSoModel->retrieve_all_kategori();
	...
	function getAllByCustomer_get($supp_id)
	{
		...
		$retrieve = $this->SlsSoModel->retrieve_all_by_customer($supp_id);
	...
	function getAllSOReleaseByCustomer_get($supp_id)
	{
		...
		$retrieve = $this->SlsSoModel->retrieve_all_so_release_by_customer($supp_id);
	...
	function getAllPSOReleaseByCustomerBlmClose_get($supp_id)
	{
		...
		if (!empty($retrieve)) {
			...
			foreach ($retrieve as $key => $ret) {
				...
				$sqlCek = "Select a.id, f.nama as lokasi, a.tanggal,a.no_so,a.catatan,b.id, b.item_id,c.kode as kode_item,c.nama as nama_item,d.kode as uom, b.qty,IFNULL(e.qty_terima, 0)as qty_sudah_terima,
	...
	function index_post()
	{
		...
		//$input['no_so'] = $this->autonumber->Sales_order($input['lokasi_pp_id']['id'], $input['tanggal'], $input['customer_id']['id']);
		// $input['no_so'] = $this->autonumber->Sales_order($input['lokasi_id']['id'], $input['tanggal'], $input['lokasi_pp_id']['id']);
		$input['no_so'] = $this->autonumber->sales_order($input['lokasi_id']['id'], $input['tanggal']);
		$res =  $this->SlsSoModel->create($input);
	...
	function simpan_pembayaran_post()
	{
		...
		$input['dibuat_oleh'] = $this->user_id;
		$this->load->library('Autonumber');
		$res =  $this->SlsSoModel->create_pembayaran($input);
		// $this->set_response(array("status" => "OK", "data" => $input['no_so']), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function update_pembayaran_put($segment_3 = '')
	{
		...
		$input['diubah_oleh'] = $this->user_id;
		$id = (int)$segment_3;
		$so = $this->SlsSoModel->retrieve_pembayaran_by_id($id);
		if (empty($so)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}

		$res =   $this->SlsSoModel->update_pembayaran($so['id'], $input);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function update_invoice_put($segment_3 = '')
	{
		...
		$input['diubah_oleh'] = $this->user_id;
		$id = (int)$segment_3;
		$so = $this->SlsSoModel->retrieve_invoice_by_id($id);
		if (empty($so)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}

		$res =   $this->SlsSoModel->update_invoice($so['id'], $input);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function index_put($segment_3 = '')
	{
		...
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->put()), 'entity' => 'sls_so', 'action' => 'edit', 'entity_id' => $id, 'key_text' => $so['no_so']);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
	...
	function revisi_put($segment_3 = '')
	{
		...
		if (!empty($so)) {
			$penerimaan = $this->db->query("SELECT b.so_id,a.* FROM sls_so_ht a
		...
		$res =   $this->SlsSoModel->revisi($so['id'], $input);
		if (!empty($res)) {
			...
			if ($so['is_revisi'] != 1) {
				if ($karyawan_id) {
					...
					<br>
					No PO : ' .	$so['no_so'] . ' direvisi dan diajukan ulang kepada Anda.<br> 
	...
	function closing_post($segment_3 = '')
	{
		...
		$this->db->query("UPDATE sls_so_ht  SET `status`='" . $input['status'] . "', `diubah_tanggal`='" . $input['diubah_tanggal'] . "', `diubah_oleh`=" . $input['diubah_oleh'] . " WHERE id=" . $id);
	...
	function hapus_pembayaran_delete($segment_3 = '')
	{
		...
		$res =  $this->SlsSoModel->delete_pembayaran($so['id']);
		if (!empty($res)) {
			/* start audit trail */
	...
	function hapus_invoice_delete($segment_3 = '')
	{
		...
		$res =  $this->SlsSoModel->delete_invoice($so['id']);
		if (!empty($res)) {
			/* start audit trail */
	...
	function approval_post($segment_3 = '')
	{
		...
		$retrieve =   $this->SlsSoModel->approval($so['id'], $input);
		if ($retrieve) {
			if ($input['karyawan_id']['id']) {
				...
				if ($karyawan['email'] || $karyawan['email'] != '') {
					...
					$email_body    = 'Hallo, Bpk/Ibu. ' . $karyawan['nama'] . '! <br>
					<br>
					No PO : ' .	$so['no_so'] . ' diajukan kepada Anda.<br> 
					Silakan melakukan approve pada Aplikasi Antech Sistem melalui   <a href="http://erp.dpaplant.com" target="_blank">AnTech Plantation - DPA</a>. 
					<br> <br> Salam 
					 <br> (Antech Sistem)';
					$this->email->set_newline("\r\n");
				...
				if ($usr['fcm_token']) {
					$title = "Approval PO";
					$message = "Hallo, Bpk/Ibu. " . $karyawan['nama'] . "!. No PO : " .	$so['no_so'] . " diajukan kepada Anda.";
			...
			if ($so['status'] == 'RELEASE') {
				...
				if ($res_email) {
					...
					<br>
					No PO : ' .	$so['no_so'] . ' Sudah Di-Release.<br> 
	...
	function reject_post($segment_3 = '')
	{
		...
		$retrieve =   $this->SlsSoModel->reject($so['id'], $input);
		$this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
	}
	...
	function posting_post($segment_3 = null)
	{
		...
		$res = $this->SlsSoModel->posting($id, $data);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id' => $id)), 'entity' => 'sls_so', 'action' => 'posting', 'entity_id' => $id, 'key_text' => $so['no_so']);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
	...
	{
		$retrieve = array();
		$dtl = $this->SlsSoModel->retrieve_so_dtl($so_id);
		$retrieve['dtl'] = $dtl;
		...
		$ret_so_ht = $this->db->query("select * from sls_so_ht where id= " . $so_id)->row_array();
	...
	{
		$retrieve = $this->SlsSoModel->retrieve_so_dtl_blm_terkirim($so_id);
	...
	{
		$retrieve = array();
		$dtl = $this->SlsSoModel->retrieve_so_dtl_sdh_terkirim($so_id);
		$retrieve['dtl'] = $dtl;
		$retrieve['AKUN_PPN'] = $this->db->query("select * from acc_auto_jurnal where kode='PPN_KELUARAN'")->row_array()['acc_akun_id'];
	...
	function print_slip_get($segment_3 = '')
	{
		...
		$queryDetail = "SELECT a.*,b.kode as kode_barang,b.nama as nama_barang,c.kode AS uom FROM sls_so_dt a
		...
		$html = $this->load->view('SlsSO_Slip', $data, true);
	...
	function print_slip_invoice_get($segment_3 = '')
	{
		...
		$html = $this->load->view('SlsSoSlipInvoice', $data, true);
	...
	function print_slip_html_get($segment_3 = '')
	{
		...
		$html = $this->load->view('SlsSo_laporan', $data, true);
	...
	function print_slip_cek_harga_get($segment_3 = '')
	{
		...
		foreach ($dataDetail as $key => $value) {
			...
			$queryLastPO = "select a.no_so,a.tanggal,c.nama_customer ,b.harga from sls_so_ht a 
			...
			if ($last_po) {
				$dataDetail[$key]['last_no_so'] = $last_po['no_so'];
		...
		$html = $this->load->view('SlsSo_laporan_cek_harga', $data, true);
	...
	function laporan_Detail_So_post()
	{
		...
		foreach ($dataBkm as $key => $hd) {
			...
			b.status
			FROM sls_so_dt a
			INNER JOIN sls_so_ht b ON a.so_hd_id=b.id
	...
	function laporanso_by_approval_post()
	{
		...
		foreach ($dataSt as $key => $hd) {
			$querydetail = "SELECT a.*, b.nama AS lokasi, c.nama_customer as nama_customer FROM sls_so_ht a
			LEFT JOIN gbm_organisasi b ON a.lokasi_id=b.id
			LEFT JOIN gbm_customer c ON a.customer_id=c.id
	...
	function laporanso_by_header_post()
	{
		...
		$queryhead = "SELECT c.no_so AS no_so, c.id AS id  FROM sls_so_dt a
		INNER JOIN sls_so_ht c ON a.so_hd_id=c.id
		where c.tanggal between  '" . $tgl_mulai . "' and  '" . $tgl_akhir . "'	
		and c.status='RELEASE'
		GROUP BY c.no_so, c.id
		";
	...
	function laporan_so_rekap()
	{
		...
		foreach ($dataPo  as $key => $po) {
			...
			$no_surat_jalan = '';
			$qty_terima = 0;
			$res_penerimaan = $this->db->query(" 	SELECT a.* FROM inv_pengiriman_so_ht a INNER JOIN sls_so_ht b ON a.po_id=b.id
