...
class KlnObatRawatJalan extends  BD_Controller
{
	public $user_id;
	...
	{
		parent::__construct();
		$this->load->model('KlnObatRawatJalanModel');
	...
	{
		$id = $segment_3;
		$retrieve = $this->KlnObatRawatJalanModel->retrieve($id);
		$retrieve['detail'] = $this->KlnObatRawatJalanModel->retrieve_detail($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->KlnObatRawatJalanModel->retrieve_all_kategori();
	...
	function index_post()
	{
		...
		$res =  $this->KlnObatRawatJalanModel->create($input);
		// $this->set_response(array("status" => "OK", "data" => $input['no_transaksi']), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'KlnObatRawatJalan', 'action' => 'new', 'entity_id' => $res);
	...
	function index_put($segment_3 = '')
	{
		...
		$id = (int)$segment_3;
		$kategori = $this->KlnObatRawatJalanModel->retrieve($id);
		if (empty($kategori)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}

		$res =   $this->KlnObatRawatJalanModel->update($kategori['id'], $input);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->put()), 'entity' => 'KlnObatRawatJalan', 'action' => 'edit', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $input['no_transaksi']), REST_Controller::HTTP_CREATED);
		} else {
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->KlnObatRawatJalanModel->delete($kategori['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id' => $id)), 'entity' => 'KlnObatRawatJalan', 'action' => 'delete', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
	...
	function posting_post($segment_3 = null)
	{
		...
		$retrieve_header = $this->KlnObatRawatJalanModel->retrieve($id);
		...
		$retrieve_detail = $this->KlnObatRawatJalanModel->retrieve_detail($id);
		...
		$retrieve_akun = $this->db->query("SELECT * FROM acc_auto_jurnal
		 where kode='KlnObatRawatJalanUSTMENT_STOK'")->row_array();
		...
		// return;
		$this->AccJurnalModel->delete_by_ref_id_and_modul($retrieve_header['id'], 'KlnObatRawatJalan_STOK');
		// Data HEADER
		$this->load->library('Autonumber');
		$no_jurnal = $this->autonumber->jurnal_auto($retrieve_header['lokasi_id'], $retrieve_header['tanggal'], 'KlnObatRawatJalan');
		...
		$dataH = array(
			...
			'tipe_jurnal' => 'AUTO',
			'modul' => 'KlnObatRawatJalan_STOK',
			'keterangan' => 'KlnObatRawatJalan_STOK',
			'is_posting' => 1,
		);
		...
		$res = $this->KlnObatRawatJalanModel->posting($id, $data);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function print_slip_get($segment_3 = '')
	{
		...
		$queryHeader = "SELECT a.*,b.no_transaksi AS no_rawat_jalan,c.nama AS nama_pasien,d.nama AS nama_dokter
		 FROM kln_obat_rawat_jalan_ht a 
		...
		$html = $this->load->view('KlnSlipObat', $data, true);
	...
	function laporan_detail_post()
	{
		...
		FROM kln_obat_rawat_jalan_ht a 
		INNER JOIN kln_obat_rawat_jalan_dt b 
		ON a.id=b.kln_obat_id 
		INNER JOIN kln_rawat_jalan r ON a.rawat_jalan_id=r.id
		...
		$html = $this->load->view('KlnPemakaianObatDetail', $data, true);
