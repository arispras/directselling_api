...
class KlnRawatJalan extends BD_Controller
{
	public $user_id;
	...
	{
		parent::__construct();
		$this->load->model('KlnRawatJalanModel');
	...
	public function list_post()
	{
		...
		(SELECT pasien_id, COUNT(pasien_id) AS visit_count , MAX(tanggal) AS last_visit FROM kln_rawat_jalan
	...
	{
		$id = $segment_3;
		$retrieve = $this->KlnRawatJalanModel->retrieve($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->KlnRawatJalanModel->retrieve_all();
	...
	function getAllbyPasien_get($pasien_id)
	{
		...
		$retrieve = $this->KlnRawatJalanModel->retrieve_all_by_pasien($pasien_id);
	...
	function create_post()
	{
		...
		$res =  $this->KlnRawatJalanModel->create($data);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'kln_rawat_jalan', 'action' => 'new', 'entity_id' => $res);
	...
	{
		$id = (int)$segment_3;
		$item = $this->KlnRawatJalanModel->retrieve($id);
		if (empty($item)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}
		...
		$res =   $this->KlnRawatJalanModel->update($item['id'], $data);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'kln_rawat_jalan', 'action' => 'edit', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->KlnRawatJalanModel->delete($item['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id' => $id)), 'entity' => 'kln_rawat_jalan', 'action' => 'delete', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
	...
	{
		$id = (int) $segment_3;
		$retrieve_header = $this->KlnRawatJalanModel->retrieve($id);
		...
		$res = $this->KlnRawatJalanModel->posting($id, $input);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function print_slip_get($segment_3 = '')
	{
		...
		$html = $this->load->view('KlnSlipRawatjalan', $data, true);
	...
	function laporan_detail_post()
	{
		...
		$html = $this->load->view('KlnRawatJalanDetail', $data, true);
