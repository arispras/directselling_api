...
class Karyawan extends BD_Controller
{
	...
	function SaldoCuti_post()
	{
		...
		$queryCuti = "SELECT a.nip,a.nama,c.nama as lokasi, IFNULL( SUM(b.jumlah),0) as saldo,a.id from karyawan a 
		...
		GROUP by a.nip, a.nama,c.nama,a.id
		";
		
		$resOrg = $this->db->query("select * from gbm_organisasi where id=" . $lokasi_id . "")->row_array();
		$filter_lokasi = $resOrg['nama'];

		$dataCuti=array();
		$dataSaldoCuti = $this->db->query($queryCuti)->result_array();
		
		foreach ($dataSaldoCuti as $key => $saldoCuti) {
			$queryAbsenCuti = "SELECT a.karyawan_id, SUM(b.jumlah_hk)AS jum_cuti FROM payroll_absensi a INNER JOIN hrms_jenis_absensi b ON a.jenis_absensi_id=b.id 
			WHERE b.kode IN('CT','CB','CM')
			and  a.karyawan_id = " . $saldoCuti['id'] . "
			";
			$jumSaldoCuti=$saldoCuti['saldo']?$saldoCuti['saldo']:0;
			$dataAbsenCuti = $this->db->query($queryAbsenCuti)->row_array();
			$absenCuti=$dataAbsenCuti['jum_cuti']?$dataAbsenCuti['jum_cuti']:0;
			$SaldoAkhirCuti=$jumSaldoCuti-$absenCuti;
			$dataCuti[]=array('nip'=>$saldoCuti['nip'],'nama'=>$saldoCuti['nama'],'lokasi'=>$saldoCuti['lokasi'],'saldo'=>$SaldoAkhirCuti);

		} 
		// var_dump($dataCuti);return;
		$data['bkm'] = 	$dataCuti;
	...
	function index_post()
	{
		...
		$is_dokter   = $this->post('is_dokter', TRUE);
		$is_perawat   = $this->post('is_perawat', TRUE);
	...
	public	function index_put($segment_3 = '')
	{
		...
		$is_dokter = $this->put('is_dokter', TRUE);
		$is_perawat = $this->put('is_perawat', TRUE);
		# update Karyawan
		...
		/* start audit trail */
		$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->put()), 'entity' => 'hrms_karyawan', 'action' => 'edit', 'entity_id' => $Karyawan_id );
		$this->db->insert('fwk_user_audit', $audit);
		/* end audit trail */	
		$this->set_response(array("status" => "OK", "data" => $Karyawan_id), REST_Controller::HTTP_CREATED);
	}
	...
	function getAllDokter_get()
	{

		$retrieve = $this->KaryawanModel->retrieve_all_Karyawan_dokter();
	...
	function getAllPerawat_get()
	{

		$retrieve = $this->KaryawanModel->retrieve_all_Karyawan_Perawat();
...
";
  ...
  ';
	...
	function edit_picture__old_post($segment_3 = '')
	{
