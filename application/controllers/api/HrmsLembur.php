...
class HrmsLembur extends BD_Controller
{
	public $user_id;
	...
	public function list_post()
	{
		...
		$query  = "select a.*,b.nip,b.nama from payroll_lembur a inner join karyawan b on a.karyawan_id=b.id";
		$search = array('a.tanggal', 'b.nip', 'b.nama');
		$where  = null;
	...
	{
		$id = $segment_3;
		$retrieve = $this->HrmsLemburModel->retrieve($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->HrmsLemburModel->retrieve_all_item();
	...
	function create_post()
	{
		...
		$retrieve =  $this->HrmsLemburModel->create($this->post());
		if (!empty($retrieve)) {
			/* start audit trail */
	...
	function update_post($segment_3 = '')
	{
		...
		$id = (int)$segment_3;
		$item = $this->HrmsLemburModel->retrieve($id);
		if (empty($item)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}

		$res =   $this->HrmsLemburModel->update($item['id'], $this->post());
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->HrmsLemburModel->delete($item['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function hitung_lembur_post()
	{
		...
		$res =	$this->db->query("select * from payroll_basis_lembur where tipe_lembur='" . $tipe_lembur . "'
		and  basis_jam_lembur <= " . ceil($jumlah_jam) . "
		 order by basis_jam_lembur ")->result_array();
		$jml_jam =  $jumlah_jam;
		$hasil_jam = 0.0;
				...
				for ($i = 2; $i < (count($excel) + 1); $i++) {
					...
					if (!empty($data['A'])) {
						...
						if (empty($karyawan) || empty($lokasi) || empty($res_gaji)) {
							...
							# simpan data absen
							$id = $this->HrmsLemburModel->create(
								$data_save
							);
						}
					}
