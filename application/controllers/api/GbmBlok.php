...
class GbmBlok extends Rest_Controller
{
    function __construct()
    {
        parent::__construct();
		date_default_timezone_set("Asia/Jakarta");
		$this->load->model('GbmBlokModel');
    ...
    {
		$id = $segment_3;
		$retrieve = $this->GbmBlokModel->retrieve($id);
	 ...
	 function getAll_get()
	 {
		 ...
		 $retrieve = $this->GbmBlokModel->retrieve_all_item();
    ...
    function create_post()
    {
		...
		$retrieve=  $this->GbmBlokModel->create($this->post());
			$this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
    ...
    function update_post($segment_3 = '')
    {
        ...
        $id = (int)$segment_3;
        $item = $this->GbmBlokModel->retrieve( $id);
        if (empty($item)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);

        }

         $retrieve=   $this->GbmBlokModel->update($item['id'], $this->post());
			$this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
    ...
    function index_delete($segment_3 = '')
    {
        ...
        if (empty($item)) {
         ...
         $retrieve=  $this->GbmBlokModel->delete($item['id']);
			$this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
	...
	function laporan_area_get()
	{
		...
		$lap_area = $this->GbmBlokModel->laporan_area();
