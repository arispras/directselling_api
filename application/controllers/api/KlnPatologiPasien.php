...
class KlnPatologiPasien extends BD_Controller
{
	public $user_id;
	...
	{
		parent::__construct();
		$this->load->model('KlnPatologiPasienModel');
	...
	public function list_post()
	{
		...
		d.nama as nama_dokter,p.nama as nama_patologi,
		...
		from kln_patologi_pasien a 
				...
				left join kln_patologi p on a.patologi_id=p.id 
	...
	{
		$id = $segment_3;
		$retrieve = $this->KlnPatologiPasienModel->retrieve($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->KlnPatologiPasienModel->retrieve_all();
	...
	function getAllbyPasien_get($pasien_id)
	{
		...
		$retrieve = $this->KlnPatologiPasienModel->retrieve_all_by_pasien($pasien_id);
	...
	function create_post()
	{
		...
		$res =  $this->KlnPatologiPasienModel->create($data);
			if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'kln_patologi_pasien', 'action' => 'new', 'entity_id' => $res);
	...
	{
		$id = (int)$segment_3;
		$item = $this->KlnPatologiPasienModel->retrieve($id);
		if (empty($item)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}
		...
		$res =   $this->KlnPatologiPasienModel->update($item['id'], $data);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'kln_patologi_pasien', 'action' => 'edit', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->KlnPatologiPasienModel->delete($item['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id'=>$id)), 'entity' => 'kln_patologi_pasien', 'action' => 'delete', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
	...
	{
		$id = (int) $segment_3;
		$retrieve_header = $this->KlnPatologiPasienModel->retrieve($id);
		...
		$res = $this->KlnPatologiPasienModel->posting($id, $input);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function print_slip_get($segment_3 = '')
	{
		...
		$html = $this->load->view('KlnSlipPatologiPasien', $data, true);

		$filename = 'patologi_' . time();
	...
	function laporan_detail_post()
	{
		...
		foreach ($dataPo as $key => $h) {
			$q="SELECT no_rekap,tanggal,total_berat_terima FROM prc_rekap_angkut_hd where kln_patologi_pasien_id=". $h['id_kontrak']."";
		...
		$html = $this->load->view('kln_patologi_pasien_Laporan', $data, true);
