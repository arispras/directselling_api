...
class GbmKelurahan extends BD_Controller
{
	public $user_id;
	...
	function __construct()
	{
		...
		$this->load->model('GbmKelurahanModel');
	...
	public function list_post()
	{
		...
		$query  = " SELECT a.*,b.nama as kecamatan 
		from gbm_kelurahan a left join gbm_kecamatan b 
		on a.kecamatan_id=b.id
		";
	...
	public function list_mobile_post($page_no, $limit)
	{
		...
		$query  = "SELECT a.*,b.nama as kecamatan 
		from GbmKelurahan a left join kecamatan b 
		on a.kecamatan_id=b.id
		order by a.nama
	...
	{
		$retrieve = array();
		$retrieve = $this->GbmKelurahanModel->retrieve_all();
	...
	{
		$retrieve = array();
		$retrieve = $this->GbmKelurahanModel->retrieve_by_id($id);
	...
	function index_post()
	{
		...
		$res = $this->GbmKelurahanModel->create($input);
		if (!empty($res)) {
	...
	function index_put($segment_3 = null)
	{
		...
		$res = $this->GbmKelurahanModel->update($id, $data);
		if (!empty($res)) {
				$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	{
		$res = $this->GbmKelurahanModel->delete($id);
		if (!empty($res)) {
				$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
