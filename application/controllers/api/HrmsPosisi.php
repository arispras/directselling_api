...
class HrmsPosisi extends BD_Controller
{
	public $user_id;
    ...
    function __construct()
    {
		...
		$this->load->model('HrmsPosisiModel');
    ...
    public function list_post()
	{
		...
		$query  = "SELECT a.*,a.id as id, j.nama as nama_jabatan,
		...
		if (count($data['data']) > 0) {
			for ($i = 0; $i < (count($data['data'])); $i++) {
				$pos = $data['data'][$i];
				$querySkill = "SELECT   b.nama as nama_skill FROM hrms_posisi_skill a INNER JOIN hrms_skill b 
				ON a.skill_id=b.id 	where a.posisi_id=" . $pos['id'] . "";
    ...
    {
		$id = $segment_3;
		$retrieve = $this->HrmsPosisiModel->retrieve($id);
		$retrieveSkill = $this->HrmsPosisiModel->retrieve_all_by_posisi_id($id);
	 ...
	 function getAll_get()
	 {
		 ...
		 $retrieve = $this->HrmsPosisiModel->retrieve_all();
    ...
    function index_post()
    {
		...
		$retrieve=  $this->HrmsPosisiModel->create($input);
		if (!empty($retrieve)) {
			/* start audit trail */
    ...
    function index_put($segment_3 = '')
    {
        ...
        $id = (int)$segment_3;
        $gudang = $this->HrmsPosisiModel->retrieve($id);
        if (empty($gudang)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		...
		$input = $this->put();
		$input['diubah_oleh']=$this->user_id;
         $res=   $this->HrmsPosisiModel->update($gudang['id'], $input);
			// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
			if (!empty($res)) {
				/* start audit trail */
    ...
    function index_delete($segment_3 = '')
    {
        ...
        if (empty($gudang)) {
         ...
         $res=  $this->HrmsPosisiModel->delete($gudang['id']);
			// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
			if (!empty($res)) {
				/* start audit trail */
