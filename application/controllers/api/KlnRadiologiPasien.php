...
class KlnRadiologiPasien extends BD_Controller
{
	public $user_id;
	...
	{
		parent::__construct();
		$this->load->model('KlnRadiologiPasienModel');
	...
	public function list_post()
	{
		...
		d.nama as nama_dokter,p.nama as nama_radiologi,
		...
		from kln_radiologi_pasien a 
				...
				left join kln_radiologi p on a.radiologi_id=p.id 
	...
	{
		$id = $segment_3;
		$retrieve = $this->KlnRadiologiPasienModel->retrieve($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->KlnRadiologiPasienModel->retrieve_all();
	...
	function getAllbyPasien_get($pasien_id)
	{
		...
		$retrieve = $this->KlnRadiologiPasienModel->retrieve_all_by_pasien($pasien_id);
	...
	function create_post()
	{
		...
		$res =  $this->KlnRadiologiPasienModel->create($data);
			if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'kln_radiologi_pasien', 'action' => 'new', 'entity_id' => $res);
	...
	{
		$id = (int)$segment_3;
		$item = $this->KlnRadiologiPasienModel->retrieve($id);
		if (empty($item)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}
		...
		$res =   $this->KlnRadiologiPasienModel->update($item['id'], $data);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'kln_radiologi_pasien', 'action' => 'edit', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->KlnRadiologiPasienModel->delete($item['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id'=>$id)), 'entity' => 'kln_radiologi_pasien', 'action' => 'delete', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
	...
	{
		$id = (int) $segment_3;
		$retrieve_header = $this->KlnRadiologiPasienModel->retrieve($id);
		...
		$res = $this->KlnRadiologiPasienModel->posting($id, $input);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function print_slip_get($segment_3 = '')
	{
		...
		$html = $this->load->view('KlnSlipRadiologiPasien', $data, true);

		$filename = 'radiologi_' . time();
	...
	function laporan_detail_post()
	{
		...
		foreach ($dataPo as $key => $h) {
			$q="SELECT no_rekap,tanggal,total_berat_terima FROM prc_rekap_angkut_hd where kln_radiologi_pasien_id=". $h['id_kontrak']."";
		...
		$html = $this->load->view('kln_radiologi_pasien_Laporan', $data, true);
