...
class PrcRekapAngkut extends BD_Controller
{
	public $user_id;
	...
	function __construct()
	{
		...
		$this->load->model('PrcRekapAngkutModel');
	...
	public function list_post()
	{
		...
		d.no_spk AS spk,
		e.nama_supplier,
		f.no_spk as no_kontrak, f.jumlah as jumlah_sales_kontrak
		FROM prc_rekap_angkut_hd a 
		INNER JOIN gbm_organisasi b ON a.lokasi_id=b.id
		left JOIN prc_kontrak_angkut d ON a.spk_id=d.id
		INNER JOIN gbm_supplier e ON a.supplier_id=e.id
		LEFT JOIN sls_kontrak f ON a.sls_kontrak_id=f.id
		";
		$search = array('no_rekap','e.nama_supplier','d.no_spk','f.no_spk');
		$where  = null;
	...
	public function listRekapforInvoice_post($supplier_id)
	{
		$post = $this->post();
	...
	{
		$retrieve = $this->PrcRekapAngkutModel->retrieve_by_id($id);
		// $retrieve['detail'] = $this->PrcRekapAngkutModel->retrieve_detail($id);
		$dt = $this->PrcRekapAngkutModel->retrieve_detail($id);
		foreach ($dt as $key => $value) {
			if ($retrieve['sumber_timbangan']=='int'){
				$dt[$key]['total']=$dt[$key]['harga']* $dt[$key]['netto_kirim'];

			}else{
				$dt[$key]['total']=$dt[$key]['harga']* $dt[$key]['netto_customer'];
	...
	function index_post()
	{
		...
		$input['no_rekap']=$this->autonumber->prc_rekap_angkut($input['lokasi_id']['id'], $input['tanggal'],$input['supplier_id']['id'],$input['item_id']);
		//$input['no_spat'] = $this->getLastNumber('pks_pengolahan_ht', 'no_spat', 'SPAT');
		// var_dump($input);
		$res = $this->PrcRekapAngkutModel->create($input);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" =>$input['no_rekap']), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data", "error"=>$res), REST_Controller::HTTP_NOT_FOUND);
		}
	}
	...
	function index_put($segment_3 = null)
	{
		...
		$res = $this->PrcRekapAngkutModel->update($id, $data);
		if (!empty($res)) {
	...
	public function index_delete($id)
	{
		...
		$res = $this->PrcRekapAngkutModel->delete($id);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	{
		$retrieve = $this->db->query("select a.*,b.no_spk from prc_rekap_angkut_hd a inner join prc_kontrak_angkut b
		on a.spk_id=b.id
	...
	{
		$query="select a.sumber_timbangan,c.sjpp_id,d.tanggal_timbang, d.tanggal_terima,netto_customer as berat_terima, c.harga,
		d.tara_kirim,d.bruto_kirim,d.netto_kirim,d.tara_customer,d.bruto_customer,d.netto_customer		
		from prc_rekap_angkut_hd a inner join prc_kontrak_angkut b
		on a.spk_id=b.id inner join prc_rekap_angkut_dt c
		on a.id=c.rekap_id inner join  pks_timbangan_kirim_sj_vw  d
		on c.sjpp_id=d.id
		where a.id=". $id ." and a.id not in(select rekap_id from  acc_angkut_invoice_ht) 
        order by d.tanggal_timbang";
	...
	function laporan_detail_post()
	{
		...
		b.*,
		c.nama AS lokasi,
		d.nama_supplier AS customer,
		e.no_spk AS spk,
		f.nama AS item,
		g.no_surat AS no_surat,
		a.id AS id
		
		FROM prc_rekap_angkut_dt a
		LEFT JOIN prc_rekap_angkut_hd b ON a.rekap_id=b.id
		LEFT JOIN gbm_organisasi c ON b.lokasi_id=c.id
		LEFT JOIN gbm_supplier d ON b.supplier_id=d.id
	...
	function print_slip_get($segment_3 = '')
	{
		...
		$hd = $this->PrcRekapAngkutModel->print_slip_header($id);
		$data['hd'] = $hd;
		$dt = $this->PrcRekapAngkutModel->print_slip_detail($id);
		$data['dt'] = $dt;

		$html = $this->load->view('PrcSlipRekapAngkut', $data, true);

		$filename = 'report_PrcSlipRekap_' . time();
		$this->pdfgenerator->generate($html, $filename, true, 'A4', 'landscape');
		// echo $html;
	}
	function print_cover_get($segment_3 = '')
	{
		$id = (int)$segment_3;
		$data = [];
		...
		$html = $this->load->view('PrcCoverRekapAngkut', $data, true);
