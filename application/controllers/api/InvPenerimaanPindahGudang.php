...
class InvPenerimaanPindahGudang extends BD_Controller
{
	public $user_id;
	...
	function __construct()
	{
		...
		$this->load->model('InvPenerimaanPindahGudangModel');
	...
	public function list_post()
	{
		...
		FROM inv_penerimaan_pindah_gudang_ht a 
	...
	{
		$retrieve = $this->InvPenerimaanPindahGudangModel->retrieve_by_id($id);
		$retrieve['detail'] = $this->InvPenerimaanPindahGudangModel->retrieve_detail($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->InvPenerimaanPindahGudangModel->retrieve_all();
	...
	function getAllBlmTerima_get($gudang_id)
	{
		...
		where b.inv_pindah_gudang_id   is null and a.ke_gudang_id=". $gudang_id .";")->result_array();
	...
	function getTraksi_get()
	{
		...
		$retrieve = $this->InvPenerimaanPindahGudangModel->retrieve_traksi();
	...
	{
		$retrieve = $this->InvPenerimaanPindahGudangModel->retrieve_all_detail($id);
	...
	function index_post()
	{
		...
		$input['no_transaksi']=$this->autonumber->inv_penerimaan_pindah_gudang($input['lokasi_id']['id'], $input['tanggal']);
		//$input['no_spat'] = $this->getLastNumber('pks_pengolahan_ht', 'no_spat', 'SPAT');
		// var_dump($input);
		$res = $this->InvPenerimaanPindahGudangModel->create($input);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'inv_penerimaan_pindah_gudang', 'action' => 'new', 'entity_id' => $res);
	...
	function index_put($segment_3 = null)
	{
		...
		$res = $this->InvPenerimaanPindahGudangModel->update($id, $data);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->put()), 'entity' => 'inv_penerimaan_pindah_gudang', 'action' => 'edit', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
	...
	public function index_delete($id)
	{
		...
		$res = $this->InvPenerimaanPindahGudangModel->delete($id);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id'=>$id)), 'entity' => 'inv_penerimaan_pindah_gudang', 'action' => 'delete', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
	...
	function posting_post($segment_3 = null)
	{
		$id = (int)$segment_3;
		$data = $this->post();
		$retrieve_header = $this->InvPenerimaanPindahGudangModel->retrieve_by_id($id);
		$retrieve_detail = $this->InvPenerimaanPindahGudangModel->retrieve_detail($id);
		...
		$retrieve_headerPindahGudang = $this->InvPindahGudangModel->retrieve_by_id($retrieve_header['inv_pindah_gudang_id']);
		if (empty($retrieve_headerPindahGudang)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Transaksi Pindah Gudang Tidak ada"), REST_Controller::HTTP_OK);
				return;

		}else{
			if ($retrieve_headerPindahGudang['is_posting']!=1) {
				$this->set_response(array("status" => "NOT OK", "data" => "Transaksi Pindah Gudang Blm Diposting"), REST_Controller::HTTP_OK);
				return;
		...
		if ($retrieve_header['tipe'] == 'EXT') {
			...
			where id=" . $retrieve_header['gudang_id'] . "")->row_array()['parent_id'];
			...
			$dataH = array(
				...
				'tipe_jurnal' => 'AUTO',
				'modul' => 'INV_PENERIMAAN_PINDAH_GUDANG',
				'keterangan' => 'INV_PENERIMAAN_PINDAH_GUDANG',
				'is_posting' => 1
			);
			...
			foreach ($retrieve_detail as $key => $value) {
				$inv_tr = $this->db->query("select * from inv_transaksi_harian  where tipe='PINDAH_GUDANG' and ref_id=" . $retrieve_header['inv_pindah_gudang_id'] . "
			...
			//   and item_id=" . $value['item_id'] . "")->row_array();
			  $avg_price = $inv_tr['nilai_keluar']/$inv_tr['qty_keluar'];
				...
				$dataDr = array(
					...
					'debet' => $nilai,
					'kredit' => 0,
					'ket' => 'Penerimaan Pindah Gudang Item:' . $value['nama_barang'] . ', qty:' . $value['qty'] . ' ' . $value['uom'],
		...
		$res = $this->InvPenerimaanPindahGudangModel->posting($id, $data);
		if (!empty($res)) {
			/* start audit trail */
	...
	function print_slip_get($segment_3 = '')
	{
		...
		b.nama as lokasi, 
		c.nama as gudang, 
		d.nama as dari_gudang 
		FROM inv_penerimaan_pindah_gudang_ht a
		INNER JOIN gbm_organisasi b ON a.lokasi_id=b.id
		INNER JOIN gbm_organisasi c ON a.gudang_id=c.id
		INNER JOIN gbm_organisasi d ON a.dari_gudang_id=d.id WHERE a.id=" . $id . "";
		...
		c.nama as satuan, 
		b.kode as kode  
		FROM inv_penerimaan_pindah_gudang_dt a
		INNER JOIN inv_item b on a.item_id=b.id
		INNER JOIN gbm_uom c on b.uom_id=c.id WHERE a.inv_penerimaan_pindah_gudang_id=" . $id . "";
		...
		$html = $this->load->view('InvSlipPenerimaanPindahGudang', $data, true);
	...
	function laporan_po_detail_post()
	{
		...
		LEFT JOIN inv_penerimaan_pindah_gudang_ht b on a.inv_penerimaan_pindah_gudang_id=b.id
		LEFT JOIN inv_item c on a.item_id=c.id
		LEFT JOIN gbm_organisasi e on b.dari_gudang_id=e.id
		LEFT JOIN gbm_organisasi f on b.gudang_id=f.id
		...
		$html = $this->load->view('Inv_Penerimaan_Pindah_Gudang_Laporan', $data, true);
