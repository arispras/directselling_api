...
class KlnInvoiceRawatInap extends  BD_Controller
{
	public $user_id;
	...
	{
		parent::__construct();
		$this->load->model('KlnInvoiceRawatInapModel');
	...
	public function list_post()
	{
		...
		from kln_invoice_rawat_inap_ht a
	...
	{
		$id = $segment_3;
		$retrieve = $this->KlnInvoiceRawatInapModel->retrieve($id);
		$retrieve['detail'] = $this->KlnInvoiceRawatInapModel->retrieve_detail($id);
		$retrieve['pembayaran'] = $this->KlnInvoiceRawatInapModel->retrieve_detail_pembayaran($id);
		if (!empty($retrieve)) {
	...
	function getAll_get()
	{
		...
		$retrieve = $this->KlnInvoiceRawatInapModel->retrieve_all_kategori();
	...
	function index_post()
	{
		...
		$res =  $this->KlnInvoiceRawatInapModel->create($input);
		// $this->set_response(array("status" => "OK", "data" => $input['no_transaksi']), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'KlnInvoiceRawatInap', 'action' => 'new', 'entity_id' => $res);
	...
	function index_put($segment_3 = '')
	{
		...
		$id = (int)$segment_3;
		$kategori = $this->KlnInvoiceRawatInapModel->retrieve($id);
		if (empty($kategori)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}

		$res =   $this->KlnInvoiceRawatInapModel->update($kategori['id'], $input);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->put()), 'entity' => 'KlnInvoiceRawatInap', 'action' => 'edit', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $input['no_transaksi']), REST_Controller::HTTP_CREATED);
		} else {
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->KlnInvoiceRawatInapModel->delete($kategori['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id' => $id)), 'entity' => 'KlnInvoiceRawatInap', 'action' => 'delete', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
	...
	function posting_post($segment_3 = null)
	{
		...
		$retrieve_header = $this->KlnInvoiceRawatInapModel->retrieve($id);
		...
		$retrieve_detail = $this->KlnInvoiceRawatInapModel->retrieve_detail($id);
		...
		$retrieve_akun = $this->db->query("SELECT * FROM acc_auto_jurnal
		 where kode='KlnInvoiceRawatInapUSTMENT_STOK'")->row_array();
		...
		// return;
		$this->AccJurnalModel->delete_by_ref_id_and_modul($retrieve_header['id'], 'KlnInvoiceRawatInap_STOK');
		// Data HEADER
		$this->load->library('Autonumber');
		$no_jurnal = $this->autonumber->jurnal_auto($retrieve_header['lokasi_id'], $retrieve_header['tanggal'], 'KlnInvoiceRawatInap');
		...
		$dataH = array(
			...
			'tipe_jurnal' => 'AUTO',
			'modul' => 'KlnInvoiceRawatInap_STOK',
			'keterangan' => 'KlnInvoiceRawatInap_STOK',
			'is_posting' => 1,
		);
		...
		$res = $this->KlnInvoiceRawatInapModel->posting($id, $data);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function print_slip_get($segment_3 = '')
	{
		...
		FROM kln_invoice_rawat_inap_ht a 
		left JOIN kln_rawat_inap b ON a.rawat_inap_id=b.id 
		...
		c.nama as uom
		FROM kln_invoice_rawat_inap_dt a 
		...
		$queryPembayaran = "	SELECT a.*,b.nama,b.jenis from kln_invoice_rawat_inap_bayar a 
	...
	function laporan_detail()
	{
		...
		f.nip AS nip_dokter,f.nama AS nama_dokter,g.nama AS nama_poli from kln_invoice_rawat_inap_ht a 
		INNER JOIN kln_rawat_inap d ON a.rawat_inap_id=d.id
		...
		foreach ($dataHd  as $key => $value) {
			...
			f.nip AS nip_dokter,f.nama AS nama_dokter,g.nama AS nama_poli from kln_invoice_rawat_inap_ht a 
			INNER JOIN kln_invoice_rawat_inap_dt b ON a.id=b.invoice_id
		...
		$html = $this->load->view('KlnInvoiceRawatInapDetail', $data, true);
	...
	function laporan_detail_by_jenis_biaya()
	{
		...
		$result = array();
		$result = [];
		$queryHd = "SELECT Distinct c.id,c.kode as kode_biaya,c.nama as nama_biaya  from kln_invoice_rawat_inap_ht a 
			INNER JOIN kln_invoice_rawat_inap_dt b ON a.id=b.invoice_id
			INNER JOIN acc_kegiatan c ON b.biaya_id=c.id
			INNER JOIN kln_rawat_inap d ON a.rawat_inap_id=d.id
			INNER JOIN kln_pasien e ON d.pasien_id=e.id
		...
		$html = $this->load->view('KlnInvoiceRawatInapDetailByBiaya', $data, true);
	...
	function laporan_detail_by_tipe_bayar()
	{
		...
		from kln_invoice_rawat_inap_ht a 
		INNER JOIN kln_invoice_rawat_inap_bayar b ON a.id=b.invoice_id
		...
		$html = $this->load->view('KlnInvoiceRawatInapDetailByTipeBayar', $data, true);
