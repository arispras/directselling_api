...
class GbmKabupaten extends BD_Controller
{
	public $user_id;
	...
	function __construct()
	{
		...
		$this->load->model('GbmKabupatenModel');
	...
	public function list_post()
	{
		...
		from gbm_kabupaten a left join gbm_provinsi b 
	...
	public function list_mobile_post($page_no, $limit)
	{
		...
		$query  =" SELECT a.*,b.nama as provinsi 
		from GbmKabupaten a left join provinsi b 
		on a.provinsi_id=b.id
		order by a.nama
	...
	{
		$retrieve = array();
		$retrieve = $this->GbmKabupatenModel->retrieve_all();
	...
	{
		$retrieve = array();
		$retrieve = $this->GbmKabupatenModel->retrieve_by_id($id);
	...
	function index_post()
	{
		...
		$res = $this->GbmKabupatenModel->create($input);
		if (!empty($res)) {
	...
	function index_put($segment_3 = null)
	{
		...
		$res = $this->GbmKabupatenModel->update($id, $data);
		if (!empty($res)) {
				$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	{
		$res = $this->GbmKabupatenModel->delete($id);
		if (!empty($res)) {
				$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
