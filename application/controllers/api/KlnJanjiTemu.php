...
class KlnJanjiTemu extends BD_Controller
{
	public $user_id;
	...
	{
		parent::__construct();
		$this->load->model('KlnJanjiTemuModel');
	...
	{
		$id = $segment_3;
		$retrieve = $this->KlnJanjiTemuModel->retrieve($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->KlnJanjiTemuModel->retrieve_all();
	...
	function getAllbyPasien_get($pasien_id)
	{
		...
		$retrieve = $this->KlnJanjiTemuModel->retrieve_all_by_pasien($pasien_id);
	...
	function create_post()
	{
		...
		$res =  $this->KlnJanjiTemuModel->create($data);
			if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'kln_janji_temu', 'action' => 'new', 'entity_id' => $res);
	...
	{
		$id = (int)$segment_3;
		$item = $this->KlnJanjiTemuModel->retrieve($id);
		if (empty($item)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}
		...
		$res =   $this->KlnJanjiTemuModel->update($item['id'], $data);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'kln_janji_temu', 'action' => 'edit', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->KlnJanjiTemuModel->delete($item['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id'=>$id)), 'entity' => 'kln_janji_temu', 'action' => 'delete', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
	...
	{
		$id = (int) $segment_3;
		$retrieve_header = $this->KlnJanjiTemuModel->retrieve($id);
		...
		$res = $this->KlnJanjiTemuModel->posting($id, $input);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function laporan_detail_post()
	{
		...
		foreach ($dataPo as $key => $h) {
			$q="SELECT no_rekap,tanggal,total_berat_terima FROM prc_rekap_angkut_hd where kln_janji_temu_id=". $h['id_kontrak']."";
		...
		$html = $this->load->view('kln_janji_temu_Laporan', $data, true);
