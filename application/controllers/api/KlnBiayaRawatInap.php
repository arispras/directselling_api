...
class KlnBiayaRawatInap extends  BD_Controller
{
	public $user_id;
	...
	{
		parent::__construct();
		$this->load->model('KlnBiayaRawatInapModel');
	...
	{
		$id = $segment_3;
		$retrieve = $this->KlnBiayaRawatInapModel->retrieve($id);
		// $retrieve['detail'] = $this->KlnBiayaRawatInapModel->retrieve_detail($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->KlnBiayaRawatInapModel->retrieve_all_kategori();
	...
	function index_post()
	{
		...
		$res =  $this->KlnBiayaRawatInapModel->create($input);
	...
	function index_put($segment_3 = '')
	{
		...
		$id = (int)$segment_3;
		$kategori = $this->KlnBiayaRawatInapModel->retrieve($id);
		if (empty($kategori)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}

		$res =   $this->KlnBiayaRawatInapModel->update($kategori['id'], $input);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->KlnBiayaRawatInapModel->delete($kategori['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function posting_post($segment_3 = null)
	{
		...
		$retrieve_header = $this->KlnBiayaRawatInapModel->retrieve($id);
		...
		$retrieve_detail = $this->KlnBiayaRawatInapModel->retrieve_detail($id);
		...
		// Data HEADER
		$this->load->library('Autonumber');
		$no_jurnal = $this->autonumber->jurnal_auto($retrieve_header['lokasi_id'], $retrieve_header['tanggal'], 'KlnBiayaRawatInap');
		...
		$res = $this->KlnBiayaRawatInapModel->posting($id, $data);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function print_slip_get($segment_3 = '')
	{
		...
		$queryHeader = "SELECT a.*,b.no_transaksi AS no_rawat_inap,c.nama AS nama_pasien,d.nama AS nama_dokter ,k.kode AS kode_biaya,k.nama AS nama_biaya
		FROM kln_biaya_rawat_inap a 
		...
		$html = $this->load->view('KlnSlipBiayaRawatInap', $data, true);
