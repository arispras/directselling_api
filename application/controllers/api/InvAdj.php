...
class InvAdj extends  BD_Controller
{
	public $user_id;
	...
	public function list_post()
	{
		...
		";
		$search = array('tanggal', 'no_ref', 'no_transaksi', 'catatan');
		$where  = null;
	...
	{
		$id = $segment_3;
		$retrieve = $this->InvAdjModel->retrieve($id);
		$retrieve['detail'] = $this->InvAdjModel->retrieve_detail($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->InvAdjModel->retrieve_all_kategori();
	...
	function index_post()
	{
		...
		$res =  $this->InvAdjModel->create($input);
	...
	function index_put($segment_3 = '')
	{
		...
		$id = (int)$segment_3;
		$kategori = $this->InvAdjModel->retrieve($id);
		if (empty($kategori)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}

		$res =   $this->InvAdjModel->update($kategori['id'], $input);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->InvAdjModel->delete($kategori['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function posting_post($segment_3 = null)
	{
		...
		$retrieve_header = $this->InvAdjModel->retrieve($id);
		...
		$retrieve_detail = $this->InvAdjModel->retrieve_detail($id);
		...
		// Data HEADER
		$this->load->library('Autonumber');
		$no_jurnal = $this->autonumber->jurnal_auto($retrieve_header['lokasi_id'], $retrieve_header['tanggal'], 'INVADJ');
		...
		$res = $this->InvAdjModel->posting($id, $data);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function laporan_po_detail_post()
	{
		...
		LEFT JOIN inv_adj_ht b on a.penerimaan_tanpa_po_id=b.id
