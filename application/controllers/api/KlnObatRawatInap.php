...
class KlnObatRawatInap extends  BD_Controller
{
	public $user_id;
	...
	{
		parent::__construct();
		$this->load->model('KlnObatRawatInapModel');
	...
	{
		$id = $segment_3;
		$retrieve = $this->KlnObatRawatInapModel->retrieve($id);
		$retrieve['detail'] = $this->KlnObatRawatInapModel->retrieve_detail($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->KlnObatRawatInapModel->retrieve_all_kategori();
	...
	function index_post()
	{
		...
		$res =  $this->KlnObatRawatInapModel->create($input);
		// $this->set_response(array("status" => "OK", "data" => $input['no_transaksi']), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'KlnObatRawatInap', 'action' => 'new', 'entity_id' => $res);
	...
	function index_put($segment_3 = '')
	{
		...
		$id = (int)$segment_3;
		$kategori = $this->KlnObatRawatInapModel->retrieve($id);
		if (empty($kategori)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}

		$res =   $this->KlnObatRawatInapModel->update($kategori['id'], $input);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->put()), 'entity' => 'KlnObatRawatInap', 'action' => 'edit', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$this->set_response(array("status" => "OK", "data" => $input['no_transaksi']), REST_Controller::HTTP_CREATED);
		} else {
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->KlnObatRawatInapModel->delete($kategori['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id' => $id)), 'entity' => 'KlnObatRawatInap', 'action' => 'delete', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
	...
	function posting_post($segment_3 = null)
	{
		...
		$retrieve_header = $this->KlnObatRawatInapModel->retrieve($id);
		...
		$retrieve_detail = $this->KlnObatRawatInapModel->retrieve_detail($id);
		...
		$retrieve_akun = $this->db->query("SELECT * FROM acc_auto_jurnal
		 where kode='KlnObatRawatInapUSTMENT_STOK'")->row_array();
		...
		// return;
		$this->AccJurnalModel->delete_by_ref_id_and_modul($retrieve_header['id'], 'KlnObatRawatInap_STOK');
		// Data HEADER
		$this->load->library('Autonumber');
		$no_jurnal = $this->autonumber->jurnal_auto($retrieve_header['lokasi_id'], $retrieve_header['tanggal'], 'KlnObatRawatInap');
		...
		$dataH = array(
			...
			'tipe_jurnal' => 'AUTO',
			'modul' => 'KlnObatRawatInap_STOK',
			'keterangan' => 'KlnObatRawatInap_STOK',
			'is_posting' => 1,
		);
		...
		$res = $this->KlnObatRawatInapModel->posting($id, $data);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function print_slip_get($segment_3 = '')
	{
		...
		$html = $this->load->view('KlnSlipObatRawatInap', $data, true);
