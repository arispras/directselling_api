...
class KlnKategoriRadiologi extends BD_Controller
{
	public $user_id;
    ...
    function __construct()
    {
		...
		$this->load->model('KlnKategoriRadiologiModel');
    ...
    {
		$id = $segment_3;
		$retrieve = $this->KlnKategoriRadiologiModel->retrieve($id);
	 ...
	 function getAll_get()
	 {
		 ...
		 $retrieve = $this->KlnKategoriRadiologiModel->retrieve_all();
    ...
    function index_post()
    {
		...
		$retrieve=  $this->KlnKategoriRadiologiModel->create($input);
		if (!empty($retrieve)) {
			/* start audit trail */
    ...
    function index_put($segment_3 = '')
    {
        ...
        $id = (int)$segment_3;
        $gudang = $this->KlnKategoriRadiologiModel->retrieve($id);
        if (empty($gudang)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		...
		$input = $this->put();
		$input['diubah_oleh']=$this->user_id;
         $res=   $this->KlnKategoriRadiologiModel->update($gudang['id'], $input);
			// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
			if (!empty($res)) {
				/* start audit trail */
    ...
    function index_delete($segment_3 = '')
    {
        ...
        if (empty($gudang)) {
         ...
         $res=  $this->KlnKategoriRadiologiModel->delete($gudang['id']);
			// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
			if (!empty($res)) {
				/* start audit trail */
