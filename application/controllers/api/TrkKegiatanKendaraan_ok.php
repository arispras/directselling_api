...
class TrkKegiatanKendaraan extends BD_Controller //
{
	...
	public function list_post()
	{
		...
		$query  = "SELECT a.*,b.nama as lokasi,c.nama as traksi,d.nama as mandor,e.nama as kendaraan,e.kode as kode_kendaraan FROM `trk_kegiatan_kendaraan_ht` a inner join gbm_organisasi b on a.lokasi_id=b.id
		inner join gbm_organisasi c on a.traksi_id=c.id left join karyawan d on a.mandor_id =d.id
	...
	public function start_proses_alokasi_mill($id)
	{
		...
		foreach ($res_transit_account as $transit_account) {
			...
			$traksi[$transit_account['kendaraan_mesin_id']] = $nilai;
			$q = "SELECT a.kendaraan_id,c.kode as kode_kendaraan,c.nama as nama_kendaraan,b.blok_id,e.acc_akun_id,
			b.km_hm_jumlah,g.nama as mesin,e.nama as kegiatan,i.parent_id as lokasi_kerja_id ,b.ket
			FROM trk_kegiatan_kendaraan_ht a 
			inner join trk_kegiatan_kendaraan_log b on a.id=b.trk_kegiatan_kendaraan_id 
			inner join trk_kendaraan c on a.kendaraan_id =c.id 
			inner join acc_kegiatan e on b.acc_kegiatan_id=e.id 
	...
	public function laporan_log_by_tanggal_post()
	{
		...
		$html = $html . '
		<h2>Laporan Rekap Log Kendaraan/AB/Mesin</h2>
		<h3>Traksi:' . $retrieveTraksi['nama'] . ' </h3>
		...
		<tr>
			...
			<td colspan=" . (($jumlah_hari + 1) * 2) . "  style='text-align: center'> Tanggal  </td>
			
			<td rowspan=2  style='text-align: right'>Jumlah</td>
		</tr>
		";
		$html = $html . "<tr>";
		while ($d1 <= $d2) {
			...
			$html = $html . "<td colspan='2' style='text-align: center'>" . $dd . "</td>";
			$d1->modify('+1 day');
		}
		$html = $html . "</tr>";
		$d1 = new DateTime($tgl_mulai);
		while ($d1 <= $d2) {
			$ddmm = $d1->format('d-m');
			$html = $html . "<td style='text-align: right'>Status</td>";
			$html = $html . "<td style='text-align: right'>Km,Hm</td>";
			$d1->modify('+1 day');
		}
		$html = $html . "<td style='text-align: right'>Km,Hm</td>";
		$html = $html . "</tr> </thead>";
