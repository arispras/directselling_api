...
class WrkKegiatan extends BD_Controller
{
	...
	public function list_post()
	{
		...
		left join trk_kendaraan c on a.kendaraan_mesin_id=c.id 
		left join gbm_organisasi d on a.workshop_id=d.id
		...
		";
		$search = array('a.no_transaksi', 'a.tanggal', 'b.nama', 'c.nama', 'd.nama', 'c.kode', 'a.alasan');
		$where  = null;
	...
	function index_post()
	{
		...
		// $audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'wrk_kegiatan_kendaraan', 'action' => 'new', 'entity_id' => '');
		// $this->db->insert('fwk_user_audit', $audit);
		$details = $input['details_kegiatan'];
		foreach ($details as $key => $value) {
			$blok_id = $value['blok_id']['id'];
			$kegiatan_id = $value['kegiatan_id']['id'];
			$traksi_id =  $value['kendaraan_id']['id'];
		...
		if (!empty($res)) {
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'wrk_kegiatan_kendaraan', 'action' => 'new', 'entity_id' => $res);
			$this->db->insert('fwk_user_audit', $audit);
	...
	function index_put($segment_3 = null)
	{
		...
		foreach ($details as $key => $value) {
			$blok_id = $value['blok_id']['id'];
			$kegiatan_id = $value['acc_kegiatan_id']['id'];
			$traksi_id =  $value['kendaraan_id']['id'];
		...
		if (!empty($res)) {
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->put()), 'entity' => 'wrk_kegiatan_kendaraan', 'action' => 'edit', 'entity_id' => $res);
			$this->db->insert('fwk_user_audit', $audit);
	...
	public function index_delete($id)
	{
		...
		if (!empty($res)) {
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id' => $id)), 'entity' => 'wrk_kegiatan_kendaraan', 'action' => 'delete', 'entity_id' => $id);
			$this->db->insert('fwk_user_audit', $audit);
	...
	function posting_post($segment_3 = null)
	{
	   ...
	   left join trk_kendaraan d on a.kendaraan_mesin_id =d.id where b.wrk_kegiatan_id=" . $id . "";
			...
			if ($resJurnalTemp) {
				...
				foreach ($resJurnalTemp as $key => $JurnalTemp) {
					...
					if ($JurnalTemp['kendaraan']) {
						$ket = 'WORKSHOP (Upah/premi) Kendaraan: ' . $JurnalTemp['kendaraan'];
					} else {
						$ket = 'WORKSHOP (Upah/premi)';
					}
					if ($JurnalTemp['hk'] > 0) {
		...
		$res = $this->WrkKegiatanModel->posting($id, $input);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	public function start_proses_alokasi_mill($id)
	{
		...
		if ($res_transit_account) {
			...
			b.blok_id, b.jumlah_jam,b.acc_kegiatan_id as kegiatan_id
			FROM wrk_kegiatan_ht a 
	...
	public function start_proses_alokasi_estate($id)
	{
		...
		if ($res_transit_account) {
			...
			a.lama_perbaikan,b.ket,
			c.acc_akun_id,d.kode as kode_akun,d.nama as nama_akun,b.kendaraan_id,
			b.blok_id, b.jumlah_jam,e.tahuntanam,e.statusblok,b.acc_kegiatan_id as kegiatan_id
			FROM wrk_kegiatan_ht a 
			inner join wrk_kegiatan_log b on b.wrk_kegiatan_id=a.id
			inner  join acc_kegiatan c on b.acc_kegiatan_id=c.id
			inner join acc_akun d on c.acc_akun_id=d.id
