...
class PksSounding extends BD_Controller
{
    public $user_id;
    ...
    {
        parent::__construct();
		$this->load->model('PksSoundingModel');
    ...
    {
		$id = $segment_3;
		$retrieve = $this->PksSoundingModel->retrieve($id);
	 ...
	 function getAll_get()
	 {
		 ...
		 $retrieve = $this->PksSoundingModel->retrieve_all();
		...
		function processSounding_post()
		{
			$input=$this->post();
		 $retrieve = $this->PksSoundingModel->process_sounding( $input );		 
		 if (!empty($retrieve)) {
			 $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		 } else {
			 $this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
    ...
    function create_post()
    {
		...
		$data['no_transaksi']=$this->autonumber->pks_sounding($data['mill_id']['id'],$data['tanggal'],$data['supplier_id']['id']);
		
		$validate = $this->PksSoundingModel->check_tanki($data['tanki_id'],$data['tanggal']);
		if ( $validate ) {
		// if (true) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'pks_sounding', 'action' => 'new', 'entity_id' => $validate);
			$this->db->insert('fwk_user_audit', $audit);
			/* end audit trail */
			$retrieve = $this->PksSoundingModel->create($data);
			// $idProduksi = $this->PksSoundingModel->createProduksi($retrieve, $data);
			$this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		}else {
			$this->set_response(array("status" => "NOT OK", "data" => 'Sounding Tangki tsb Sudah Ada Untuk Hari Ini'), REST_Controller::HTTP_OK);
		}
	}
    ...
    function update_post($segment_3 = '')
    {
        ...
        $id = (int)$segment_3;
        $item = $this->PksSoundingModel->retrieve( $id);
        if (empty($item)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		...
		$data['diubah_oleh']=$this->user_id;
         $res=   $this->PksSoundingModel->update($item['id'], $data);
        //  $retrieve=   $this->PksSoundingModel->updateProduksi($item['id'], $data);
			// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
			if (!empty($res)) {
				/* start audit trail */
				$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->put()), 'entity' => 'pks_sounding', 'action' => 'edit', 'entity_id' => $id);
				$this->db->insert('fwk_user_audit', $audit);
				/* end audit trail */
    ...
    function index_delete($segment_3 = '')
    {
        ...
        if (empty($item)) {
         ...
         $res=  $this->PksSoundingModel->delete($item['id']);
         $retrieve=  $this->PksSoundingModel->deleteProduksi($item['id']);
			// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
			if (!empty($res)) {
				/* start audit trail */
				$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id'=>$id)), 'entity' => 'pks_sounding', 'action' => 'delete', 'entity_id' => $id);
				$this->db->insert('fwk_user_audit', $audit);
				/* end audit trail */
				$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
			} else {
				...
				for ($i = 2; $i < (count($excel) + 1); $i++) {
					...
					if (!empty($data['A'])) {
						...
						if (empty($karyawan)) {
							...
							# simpan data absen
							$id = $this->PksSoundingModel->create(
								$data_save
							);
						}
					}
