...
class AccApInvoice extends BD_Controller //Rest_Controller
{
	public $user_id;
	...
	public function list_post()
	{
		...
		$search = array('a.no_invoice', 'a.no_invoice_supplier', 'a.tanggal', 'b.nama', 'c.nama_supplier', 'a.deskripsi', 'd.no_po');
		// $search ='';
		$where  = null;
	...
	{
		$retrieve = array();
		$retrieve = $this->AccApInvoiceModel->retrieve_by_id($id);
		$retrieve_detail = $this->AccApInvoiceModel->retrieve_detail($id);
		//$detail = array();
		// foreach ($retrieve_detail as $key => $value) {
		// 	$dtl = array();
		// 	$retrieve_denda = $this->AccApInvoiceModel->retrieve_detail_denda($value['id']);
		// 	$dtl = $value;
		// 	$dtl['denda'] = array();
	...
	{
		$retrieve = $this->AccApInvoiceModel->retrieve_detail($id);
		if (!empty($retrieve)) {
			$this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function index_post()
	{
		...
		$input['no_invoice'] = $this->autonumber->acc_ap_invoice($input['lokasi_id']['id'], $input['tanggal']);

		$res = $this->AccApInvoiceModel->create($input);
		if (!empty($res)) {
	...
	function index_put($segment_3 = null)
	{
		...
		$res = $this->AccApInvoiceModel->update($id, $data);
		if (!empty($res)) {
	...
	function faktur_put($segment_3 = null)
	{
		...
		$res = $this->AccApInvoiceModel->update_faktur($id, $data);
	...
	public function index_delete($id)
	{
		...
		$res = $this->AccApInvoiceModel->delete($id);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function posting_post($segment_3 = null)
	{
		...
		$retrieve_header = $this->AccApInvoiceModel->retrieve_by_id($id);
		...
		// Data HEADER
		$this->load->library('Autonumber');
		$no_jurnal = $this->autonumber->jurnal_auto($retrieve_header['lokasi_id'], $retrieve_header['tanggal'], 'AP');
		$dataH = array(
			'no_jurnal' => $no_jurnal,
			...
			'tipe_jurnal' => 'AUTO',
			'modul' => 'AP_INVOICE',
			'keterangan' => 'AP INVOICE:' . $retrieve_header['deskripsi'],
			'is_posting' => 1,
		...
		$dataKredit = array(
			...
			'ket' => 'AP INVOICE:' . $retrieve_header['deskripsi'],
		...
		$res = $this->AccApInvoiceModel->posting($id, $data);
		if (!empty($res)) {
			$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
		} else {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
	...
	function getAllOutstandingInvoice_get()
	{
		...
		$res = $this->db->query("SELECT a.*,a.id as invoice_id, b.nama as lokasi,
			c.nama_supplier,c.acc_akun_id as akun_supplier_id,(a.nilai_invoice-a.nilai_dibayar) as sisa
			 FROM `acc_ap_invoice_ht` a inner join gbm_organisasi b on a.lokasi_id=b.id
			inner join gbm_supplier c on a.supplier_id=c.id left join prc_po_ht d on a.po_id=d.id
	...
	function print_slip_get($segment_3 = '')
	{
		...
		$hd = $this->db->query("select a.*,c.nama_supplier, b.kode as kode_akun,b.nama as nama_akun from acc_ap_invoice_ht a left join
		acc_akun b on a.akun_supplier_id=b.id left join gbm_supplier c
		on a.supplier_id=c.id where a.id=" . $id)->row_array();
		$data['hd'] = $hd;

		$dt = $this->db->query("select a.*, b.kode as kode_akun,b.nama as nama_akun from acc_ap_invoice_dt a left join
		acc_akun b on a.acc_akun_id=b.id where a.invoice_id=" . $id)->result_array();
		$data['dt'] = $dt;

		$html = $this->load->view('AccSlipApInvoice', $data, true);
	...
	function print_slip_2_get($segment_3 = '')
	{
		...
		FROM 
		prc_po_ht a
		WHERE id=" . $hd['po_id'])->row_array();
		$data['po'] = $po;

		$podt = $this->db->query("SELECT
		a.*,
		b.nama AS item_nama,
		bb.nama AS nama_uom,
		c.no_po AS no_po,
		...
		LEFT JOIN prc_po_ht c ON a.po_hd_id=c.id
		WHERE a.po_hd_id=" . $po['id'])->result_array();
		$data['podt'] = $podt;

		$html = $this->load->view('AccSlipApInvoice2', $data, true);

		$filename = 'AccSlipApInvoice2_' . time();
		$this->pdfgenerator->generate($html, $filename, true, 'A4', 'portrait');
		echo $html;
	}
	...
	function laporan_faktur_pajak_outstanding_get($segment_3 = '')
	{
		...
		$res = $this->db->query("SELECT a.*,b.nama_supplier FROM acc_ap_invoice_ht a inner join gbm_supplier b on a.supplier_id=b.id
	...
	function laporan_ap_post()
	{
		$versi_laporan = $this->post('versi_laporan', true);
		if ($versi_laporan == 'v1') {
			$this->laporan_ap_detail();
		} else if ($versi_laporan == 'v2') {
			$this->laporan_ap_umur_hutang();
		}
	}
	function laporan_ap_detail()
	{
		error_reporting(0);
		...
		$query = "select a.*,b.nama_supplier,IFNULL( c.dibayar, 0)as dibayar,a.nilai_invoice-(IFNULL( c.dibayar, 0))as sisa   from acc_ap_invoice_ht a INNER join gbm_supplier b 
		...
		$html = $this->load->view('Acc_Ap_Laporan', $data, true);
	...
	function laporan_ap_umur_hutang()
	{
		error_reporting(0);
		...
		$query = "Select distinct a.supplier_id,b.nama_supplier  from acc_ap_invoice_ht a INNER join gbm_supplier b 
...
// exit();
		...
		$html = $this->load->view('Acc_Ap_Laporan_Umur_Hutang', $data, true);
