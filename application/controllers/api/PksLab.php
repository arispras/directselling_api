...
class PksLab extends BD_Controller// REST_Controller
{
    public $user_id;
	...
	public function list_post()
	{
		...
		$search = array('a.ffa','a.moisture','a.kadar_air','dirt','dobi','jumlah');
		$where  = null;
    ...
    {
		$id = $segment_3;
		$retrieve = $this->PksLabModel->retrieve($id);
	 ...
	 function getAll_get()
	 {
		 ...
		 $retrieve = $this->PksLabModel->retrieve_all();
    ...
    function create_post()
    {
		...
		$res = $this->PksLabModel->create($data);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		// $validate = $this->PksLabModel->check_tanki($data['tanki_id'],$data['tanggal']);
		// if ( $validate ) {
		// }else {
		// 	$this->set_response(array("status" => "NOT OK", "data" => 'Tanki Sudah Ada Hari Ini'), REST_Controller::HTTP_OK);
		// }
		if (!empty($res)) {
			/* start audit trail */
			$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->post()), 'entity' => 'pks_lab', 'action' => 'new', 'entity_id' => $res);
    ...
    function update_post($segment_3 = '')
    {
        ...
        $id = (int)$segment_3;
        $item = $this->PksLabModel->retrieve( $id);
        if (empty($item)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		...
		$data['diubah_oleh']=$this->user_id;
         $res=   $this->PksLabModel->update($item['id'], $data);
			// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
			if (!empty($res)) {
				/* start audit trail */
				$audit = array('user_id' => $this->user_id, 'desc' => json_encode($this->put()), 'entity' => 'pks_lab', 'action' => 'edit', 'entity_id' => $id);
				$this->db->insert('fwk_user_audit', $audit);
				/* end audit trail */
    ...
    function index_delete($segment_3 = '')
    {
        ...
        if (empty($item)) {
         ...
         $res=  $this->PksLabModel->delete($item['id']);
			// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
			
			if (!empty($res)) {
				/* start audit trail */
				$audit = array('user_id' => $this->user_id, 'desc' => json_encode(array('id'=>$id)), 'entity' => 'pks_lab', 'action' => 'delete', 'entity_id' => $id);
				$this->db->insert('fwk_user_audit', $audit);
				/* end audit trail */
				$this->set_response(array("status" => "OK", "data" => $res), REST_Controller::HTTP_CREATED);
			} else {
