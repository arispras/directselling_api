...
class HrmsPendapatan extends BD_Controller
{
	public $user_id;
	...
	function __construct()
	{
		...
		$this->load->model('HrmsPendapatanModel');
	...
	public function list_post()
	{
		...
		$query  = "select a.*,b.nip,b.nama,c.nama as tipe_gaji from payroll_pendapatan a 
	...
	{
		$id = $segment_3;
		$retrieve = $this->HrmsPendapatanModel->retrieve($id);
	...
	function getAll_get()
	{
		...
		$retrieve = $this->HrmsPendapatanModel->retrieve_all_item();
	...
	function create_post()
	{
		...
		$retrieve =  $this->HrmsPendapatanModel->create($this->post());
		if (!empty($retrieve)) {
			/* start audit trail */
	...
	function update_post($segment_3 = '')
	{
		...
		$id = (int)$segment_3;
		$item = $this->HrmsPendapatanModel->retrieve($id);
		if (empty($item)) {
			$this->set_response(array("status" => "NOT OK", "data" => "Tidak ada Data"), REST_Controller::HTTP_NOT_FOUND);
		}

		$res =   $this->HrmsPendapatanModel->update($item['id'], $this->post());
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
	...
	function index_delete($segment_3 = '')
	{
		...
		$res =  $this->HrmsPendapatanModel->delete($item['id']);
		// $this->set_response(array("status" => "OK", "data" => $retrieve), REST_Controller::HTTP_OK);
		if (!empty($res)) {
			/* start audit trail */
				...
				for ($i = 2; $i < (count($excel) + 1); $i++) {
					...
					if (!empty($data['A'])) {
						...
						if (empty($karyawan) || empty($lokasi) || empty($res_gaji)) {
							...
							# simpan data absen
							$id = $this->HrmsPendapatanModel->create(
								$data_save
							);
						}
					}
