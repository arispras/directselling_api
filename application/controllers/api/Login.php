...
class Login extends REST_Controller
{
	...
	public function ganti_password_post()
	{
		...
		$get_login = $this->LoginModel->retrieve($id, null,md5( $oldpass));
		// var_dump($get_login);
		if (!empty($get_login)) {
			$update_login = $this->LoginModel->update_password($get_login['id'],($newpass));
			if ($update_login) {
	...
	public function cek_aktif_user_get()
	{
		...
		$pengajar_id = $this->get('pengajar_id');
		$ortu_id = $this->get('ortu_id');
		
		$is_aktif=false;
		if (!empty($siswa_id)) {
			$res = $this->db->query("select * from siswa where id= '" . $siswa_id . "'")->row_array();
		...
		if (!empty($pengajar_id)) {
			$res = $this->db->query("select * from pengajar where id= '" . $pengajar_id . "'")->row_array();
			if (!empty($res)){
				if ($res['status_id']==1){
					$is_aktif=true;
				}
		...
		// if (!empty($ortu_id)) {
		// 	$res = $this->db->query("select * from ortu where id= '" . $ortu_id . "'")->row_array();
		// 	if (!empty($res)){
		// 		if ($res['status_id']==1){
		// 			$is_aktif=true;
		// 		}
		...
		// }
			...
			$this->set_response(array("aktif" =>$is_aktif, "message" => ''), REST_Controller::HTTP_OK);
		
	}
	function index()
	{
		//echo $this->path_sekolah;die;
		if (is_login()) {
			redirect('welcome');
		}
		...
		if ($this->form_validation->run('login') == TRUE) {
			...
			if (empty($get_login)) {
				...
				if (!empty($last_log)) {
					...
					if ($last_log['last_activity'] > $time_minus) {
						...
						if ($current_ip != $last_agent['ip'] or $current_browser != $last_agent['browser']) {
							...
							# Ditutup dulu supaya bisa login dgn akun sama berbarenagan
				...
				$data['path_sekolah'] = $this->session->userdata('db_sekolah');
	...
	function logout()
	{
		...
		$this->session->set_userdata('id_sekolah', null);
		$this->session->set_userdata('kode_sekolah', null);
		$this->session->set_userdata('nama_sekolah', null);
		$this->session->set_userdata('db_sekolah', null);
		$this->session->set_userdata('path_sekolah', null);
			...
			if ($this->form_validation->run('register/siswa') == true) {
				...
				if ($status_id == 1) {
					...
					@kirim_email('email-template-register-siswa', $username, array(
						'nama'         => $nama,
						'nama_sekolah' => get_pengaturan('nama-company', 'value')
					));
	...
	function data_onload()
	{
		...
		$return = array(
			...
			'sedang_ujian_tryout' => $this->sedang_ujian_tryout() ? '1' : '0',
		);
