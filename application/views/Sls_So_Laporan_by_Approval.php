<!DOCTYPEhtml>
	...
	<head>
		...
		<title>A.01 Sales Order per-Period</title>
	...
	<body>
		...
		<h3 class="title">A.01 Sales Order per-Period</h3>
		<br>
		...
		<!-- table 1 -->
		<table class="table-bg border" id="table_detail">
			<thead>
				<tr>
				...
				<?php foreach ($so as $key => $val) { ?>
					...
					<?php foreach ($dt as $key => $res) { ?>
						...
						<tr>
							...
							<td left width="10%" onclick="showHideRow('<?= $res['id'] ?>');"><i class="fas fa-caret-right add-btn" id='btn_<?= $res['id'] ?>'></i><?= $res['no_so'] ?></td>
							...
							<td center width="12%"><?= $res['nama_customer'] ?></td>
						...
						<tr id='hidden_row_<?= $res['id'] ?>' class="hidden_row">
							<td colspan=8>
								<?= $res['nama_customer'] ?>
							</td>
						</tr>
