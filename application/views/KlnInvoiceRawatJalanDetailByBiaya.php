<!DOCTYPEhtml>
	...
	<body>
		...
		<h3 class="title">LAPORAN INVOICE RAWAT JALAN BY JENIS BIAYA</h3>
		<br>
				...
				<?php foreach ($invoice as $key => $val) { ?>
					...
					<?php foreach ($dt as $key2 => $d) { ?>
						...
						?>
							<tr>
								<!-- <td><?= $no ?></td> -->
								<td rowspan=<?= $countRow ?> left><?= $d['nama_biaya'] ?></td>
					...
					<?php } ?>
					<tr>
						<td colspan="6" left><strong> SubTotal &nbsp;&nbsp; <?= $val['nama_biaya'] ?></strong></td>
