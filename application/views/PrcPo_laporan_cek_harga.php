		...
		<table class="table-bg border">
			...
			<tbody>
				...
				<?php foreach ($detail as $key => $val) { ?>
					<?php $no = $no + 1;
					if ($val['last_no_po']) {
						$no_po = $val['last_no_po'] . '(' . $val['last_tanggal_po'] . ')';
					} else {
						$no_po = '-';
					}
					...
					<tr>
						...
						<td center width="3%"><?= number_format($val['stok']) ?></td>
						<td center width="3%"><?= number_format($val['qty']) ?></td>
						<td center width="5%"><?= $no_po ?></td>
						<td center width="5%"><?= number_format($val['last_harga_po']) ?></td>
						<td center width="5%"><?= $val['last_supplier'] ?></td>
						<td right width="7%"> <?= number_format($val['harga']) ?></td>
						<td right width="4%"> <?= number_format($val['diskon']) ?></td>
						<td right width="10%"> <?= number_format($val['total']) ?></td>
					</tr>
				<?php } ?>
			</tbody>
			<tfoot></tfoot>
				...
				<td center>
					...
					<?php for ($i = 1; $i < 6; $i++) { ?>
						<?php if ($header['user_approve' . $i] != null) { ?>
							...
							<?php $user_note[] = $header['note_approve' . $i]; ?>
						<?php } ?>
					<?php } ?>
					<p><?= array_reverse($pembayaran)[0] ?></p>
					<p><?= array_reverse($user_jabatan)[0] ?></p>
					<p> Note : <i><?= array_reverse($user_note)[0] ?></i></p>
				</td>
			</tr>
		</table>
