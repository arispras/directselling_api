<!DOCTYPEhtml>
<html>
	<head>
		<?php 
			if ($format_laporan=='view') {
				require '_laporan_style_fix.php';
			}
			else{
				if ($format_laporan=='pdf') {
					require '__laporan_style_pdf.php';
				}	
			}
		?>
		<style>
			* body{
				font-size: 11px ;
			}
			
		</style>
		
	</head>
	<body>
		
		
		<?php require '__laporan_header.php' ?>

		<!-- <pre><?php print_r($po) ?></pre> -->

		<h3 class="title">LAPORAN RINCIAN BKM UMUM</h3>
		<br>
		
		<div class="d-flex flex-between">
			<table class="no_border" style="width:30%">
				<tr>
					<td>Lokasi</td>
					<td>:</td>
					<td><?= $filter_lokasi ?></td>
				</tr>
				<tr>
					<td>Afdeling</td>
					<td>:</td>
					<td><?= $filter_afdeling ?></td>
				</tr>
				<tr>
					<td>Periode Tanggal</td>
					<td>:</td>
					<td><?= tgl_indo($filter_tgl_awal) .' s/d '. tgl_indo($filter_tgl_akhir) ?></td>
				</tr>
			
			</table>	
		</div>

		<div class="d-flex flex-between">
			<table class="" style="width:30%">
				
				
			</table>
		</div>
		<br>
		

		<table class="table-bg border">
			<thead>
				<tr>
					<th style="width:2%">no</th>
					<th style="width:7%">No Transaksi</th>
					<th style="width:9%">Afdeling</th>
					<th style="width:7%">Tanggal</th>
					<th style="width:9%">Kegiatan</th>
					<th style="width:9%">Karyawan</th>
					<th style="width:7%">Jenis Absensi</th>
					<th style="width:4%">Hasil Kerja</th>
					<th style="width:2%">HK</th>
					<th style="width:7%">HK(RP)</th>
					<th style="width:7%">Premi(RP)</th>
					<th style="width:7%">Keterangan</th>
					<th style="width:7%">Blok</th>
					<th style="width:7%">Kendaraan</th>

				</tr>

			</thead>

			
			<tbody>
			<?php $no= 0 ?>
			<?php foreach ($po as $key=>$val) { ?> 
				
				<?php $no= $no+1 ?>
				<tr>
					<td center><?= $no ?></td>
					<td left><?= $val['no_transaksi'] ?></td>
					<td left><?= $val['afdeling'] ?></td>
					<td center><?= tgl_indo_normal($val['tanggal'])  ?></td>
					<td left><?= $val['kegiatan'] ?></td>
					<td left><?= $val['nama_karyawan'] ?></td>
					<td center><?= $val['ket_absen'] ?></td>
					<td center><?= $val['hasil_kerja'] ?></td>
					<td center><?= $val['jumlah_hk'] ?></td>
					<td right><?= number_format($val['rupiah_hk']) ?></td>
					<td right><?= number_format($val['premi']) ?></td>					
					<td left><?= $val['ket'] ?></td>
					<td left><?= $val['blok'] ?></td>
					<td left><?= $val['kode_kendaraan'] ?>-<?= $val['nama_kendaraan'] ?></td>

				</tr>
				<?php } ?>
			
				
			</tbody>
			
			<tfoot></tfoot>
		</table>
		





		<pre><?php //print_r($headeran) ?></pre>

	</body>
</html>


				
