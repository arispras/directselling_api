<!DOCTYPEhtml>
	...
	<head>
		<title>Tarik Barang  Detail</title>
	...
	<body>
		...
		<h3 class="title">Tarik Barang DETAIL</h3>
		<br>
		...
		<table class="table-bg border">
			<thead>
				<tr>
					...
					<th style="width:9%">NoTarikBarang</th>
					<th style="width:9%">Tanggal</th>
					<th style="width:9%">Angs Penarikan</th>
					...
					<th style="width:5%">Total Nilai TB</th>
					<th style="width:5%">Dp</th>
					<th style="width:5%">Nilai Piutang</th>
					<th style="width:5%">@angsuran Ditarik</th>
					<th style="width:5%">Pendapatan TB</th>
					<th style="width:5%">Pengurang Piutang</th>
				</tr>
			...
			<tbody>
				...
				?>
					...
					<?php foreach ($data as $key => $res) { ?>
						<tr>
							<?php
							$no = $no + 1;
							$pengurang_piutang = $res['total']- $res['nilai_pendapatan_tarik_barang'];
							$jum_diskon += $res['diskon'];
							$jum_dp += $res['dp'];
							$jum_total += $res['total'];	
							$jum_nilai_piutang += $res['nilai_piutang'];
							$jum_nilai_angsuran += $res['nilai_angsuran'];
							$jum_pendapatan_tarik_barang += $res['nilai_pendapatan_tarik_barang'];
							$jum_qty += $res['qty'];
							?>
							<td center> <?= $no ?> </td>
							<td left> <?= $res['no_tarik_barang'] ?></td>
							<td center><?=  tgl_indo($res['tanggal']) ?></td>	
							<td center><?=  ($res['tenor_penarikan']) ?></td>						
							...
							<td right><?= format_number_report($res['nilai_pendapatan_tarik_barang'],$format_laporan) ?></td>
							<td right><?= format_number_report($pengurang_piutang,$format_laporan) ?></td>
				...
				<tr>
					...
					<td right><?= format_number_report($jum_pendapatan_tarik_barang,$format_laporan) ?></td>
					<td right><?= format_number_report(($jum_total-$jum_pendapatan_tarik_barang),$format_laporan) ?></td>
				</tr>
