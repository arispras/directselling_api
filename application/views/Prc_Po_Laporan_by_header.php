<!DOCTYPEhtml>
	...
	<head>
		<title>A.05 Purchase Order per-Period (Detail)</title>
	...
	<body>
		...
		<h3 class="title">A.05 Purchase Order per-Period (Detail)</h3>
		<br>
				...
				<?php foreach ($po as $key => $val) { ?>
					...
					<?php foreach ($dt as $key => $res) { ?>
						...
						<tr>
							...
							<td center width="13%"><?= $res['nama_supplier'] ?></td>
							<td center width="5%"><?= $res['kode_item'] ?></td>
							<td left width="17%"><?= $res['nama_item'] ?></td>
							<td right width="5%"><?= format_number_report($res['qty'],2,$format_laporan) ?></td>
							<td right width="8%"><?= format_number_report($res['total'],2,$format_laporan) ?></td>
