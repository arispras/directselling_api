		...
		<table style="padding:10px 0;">
			<tr>
				...
				<td width="27%" style="padding:0 10px">
					...
					<table>
						<tr>
							<h1 style="font-size:15px">Invoice</h1>
						</tr>
					</table>
				</td>
			...
			<tr>
				...
				<td>
					<table>
						<!-- style="border:0px" -->
						<tr>
							<td style="border:0px" width="31%">No Rawat Inap </td>
							<td style="border:0px; padding:0;" width="2%">:</td>
							<td style="border:0px"><?= $hd['no_rawat_inap'] ?></td>
						</tr>
					</table>
				</td>
			</tr>
			...
			<tbody>
				...
				<?php foreach ($dt as $key => $val) { ?>
					...
					<tr>
						...
						<td center width="10%"><?= $val['kode_biaya'] . '-' . $val['nama_biaya'] ?></td>
						<td center width="10%"><?= $val['keterangan'] ?></td>
						<td center width="10%"><?= number_format($val['harga']) ?></td>
						<td center width="10%"><?= number_format($jumlah) ?></td>
					</tr>
				<?php } ?>

				<tr>

					<td colspan="4" center width="2%"><strong>Total</strong></td>

					<td center width="10%"><strong><?= number_format($total) ?></strong></td>
		...
		<table style="padding:10px 0;">
			<tr>
				...
				<td width="25%" style="padding:0 20px">
					...
					<table class="table-bg border">
						<tr>
							<td colspan="2" center width="2%"><strong> PEMBAYARAN</strong></td>
							
						</tr>
						<?php foreach ($bayar as $key => $val) { ?>
							<?php
							$no = $no + 1;
							$jumlah =  $val['jumlah'];
							$total = $total + $jumlah;
							?>
							<tr>
								<td center width="10%"><?= $val['nama'] ?></td>
								<td center width="10%"><?= number_format($val['jumlah']) ?></td>

							</tr>
						<?php } ?>

						<tr>
							<td  center width="2%"><strong>Jumlah Dibayar</strong></td>
							<td center width="10%"><strong><?= number_format($total) ?></strong></td>
						</tr>
